...
void MassYieldSingleStateMCFitCB1( struct Y1Sfitvar *Y1S ,long ts, const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "4", const string Trig = "",int cBinLow=0, int cBinHigh = 181, int train_state =3, int state = 1, bool fixvar = false, bool swflag = false, double cutQVP= 0.01, double bdtlow = -1., double bdthigh =1., int bdtptMin = 0, int bdtptMax = 30, string dir_opt = "" ){
  ...
  TString mainDIR_base = Form("%s/MassDist/%s/%ld%s/Cent%d-%d_Pt%d-%d_y%.1f/%d/MC_CB1" , workdir.Data(), "BDT", ts, alt.c_str(), cBinLow, cBinHigh,(int) ptMin, (int) ptMax, rapMax, state  );
  ...
  string cutAux = "";
  string auxString = "fy_2-2.4";
  cutAux = "&& ( fabs(y) > 2.0 && fabs(y) < 2.4)";
  
  RooDataSet* reducedDS = (RooDataSet*) initialDS->reduce(RooArgSet(*(works1->var("mass"))), Form("( pt >=%f && pt <=%f) && (y >= %f && y <=%f) && (cBin>=%d && cBin<%d) &&(pt1 >= %f) && (pt2 >= %f) && (eta1 >= %f && eta1 <= %f) && ( eta2 >= %f && eta2 <= %f) && (QQVtxProb > %f) && (BDT > %f && BDT < %f) %s", ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh, MupTCut, MupTCut, etaMin, etaMax, etaMin, etaMax, cutQVP, bdtlow, bdthigh, cutAux.c_str()));
  ...
  sigmaNS_1  = new RooRealVar("sigmaNS_1", "sigma of NS", 0.12, 0.01, 0.33);
  ...
  alpha = new RooRealVar("alpha", "alpha of Crystal bal1", 1.96, 0.40, 4.50 );
  n = new RooRealVar("n", "n of Crystal ball", 1.27, 0.300, 5.700);
...
//  CBNS_3 = new RooCBShape("CBNS_3", "NS Crystal ball function3", *(works1->var("mass")), meanNS, *sigmaNS_3, *alpha, *n);
  ...
  RooAddPdf* threeCBNS = new RooAddPdf("threeCBNS", "Sum of NS Crystal ball", RooArgList(*CBNS_1), RooArgList(RooConst(1)));
...
//  }
  ...
  c1->SaveAs(Form("%s/CB1_MassDistribution_%dS_pt_%d-%d_rap_%d-%d_%dbin_noWeight_MupT%s_%s_BDT_%.4f-%.4f_bdtpt_%d_%d_vp_%.4f_MC_fix%d%s.pdf",massDIR.Data(), state,(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, MupT.Data(), Trig.c_str(), bdtlow, bdthigh, bdtptMin, bdtptMax, cutQVP,(int) fixvar, auxString.c_str()));
  c1->SaveAs(Form("%s/png/MassDistribution_%dS_pt_%d-%d_rap_%d-%d_%dbin_noWeight_MupT%s_%s_BDT_%.4f-%.4f_bdtpt_%d_%d_vp_%.4f_MC_fix%d%s.png",massDIR.Data(), state,(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, MupT.Data(), Trig.c_str(), bdtlow, bdthigh, bdtptMin, bdtptMax, cutQVP, (int) fixvar, auxString.c_str()));
...
//  }
  ...
  std::string name_output = GetFit(__FITRESLATEST, true, "CB1", train_state, state,  (int) ptMin, (int) ptMax, fabs(rapMin), rapMax, string(MupT.Data()) , Trig, false, cBinLow, cBinHigh, cutQVP, true, outputts,  bdtlow, bdthigh, bdtptMin, bdtptMax, false, auxString);  
...
void MassYieldFit_BDT_MC_CB1(long _ts, const string _fname1s = "", const string _fname3s = "", const Double_t _ptMin = 0, const Double_t _ptMax = 30, const Double_t _rapMin = -2.4, const Double_t _rapMax = 2.4, const TString _MupT = "4", const string _Trig = "", int _cBinLow = 0, int _cBinHigh = 180, int _train_state =3, int _state = 1, bool _fixvar =false, bool _swflag = false, double _cutQVP = 0.01, double _bdtlow =-1., double _bdthigh = 1., int _bdtptMin =0, int _bdtptMax = 30, string dir_opt = ""){
...
//  else if( _fixvar ==false){
    {MassYieldSingleStateMCFitCB1( &Y1Svar,_ts, _fname3s, _ptMin, _ptMax, _rapMin, _rapMax, _MupT , _Trig, _cBinLow, _cBinHigh, _train_state, _state, _fixvar, _swflag, _cutQVP, _bdtlow, _bdthigh, _bdtptMin, _bdtptMax, dir_opt);}
//  }
}
