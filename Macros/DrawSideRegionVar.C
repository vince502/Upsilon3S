...
//	ROOT::EnableImplicitMT();
	...
	theobj = "QQdca";		hist_dat_full[theobj] =  new TH1D(Form("h_%s_dat_full",theobj.c_str()), "", 100,  0,  1 );
	theobj = "QQVtxProb";	hist_dat_full[theobj] =  new TH1D(Form("h_%s_dat_full",theobj.c_str()), "", 100,  0, 0.5);
...
//		hist_ratio_dat_right 	->SetStats(0);
		...
		c1->SaveAs(Form("./VarDist/Variable_dist_side_%s.pdf" , var ) );
		c1->cd();
		c1->Clear();
		leg->Clear();
	}
