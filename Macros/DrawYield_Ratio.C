...
void DrawYield_Ratio(long ts, int yield_state, int train_state){
	TH1D* hist = new TH1D(Form("Y%dS_train%d",yield_state, train_state),"",15, 0,15);
	
	TCanvas* c1 = new TCanvas("c1","",1000,600);
	c1->cd();
		int counter1 = 2;
		string typestr;
		auto ref = getDoubleRatioValue(false, {0,181},{0,30}, "CB3:CC4:FF", -1, yield_state, 1, 9999999999, false, false, 3);
		hist->SetBinContent(1,1);
		hist->SetBinError(1,ref.getError()/ref.getVal());
		for(double bdtv : {-0.9, -0.8 ,-0.6,-0.4, -0.3, -0.2,-0.05,0.0,0.05, 0.1, 0.15, 0.2, 0.25, 0.3} )
		{
			...
			RooRealVar cval;
			try{	cval = getDoubleRatioValue(false, {0,181},{0,30}, typestr, bdtv, yield_state, 1, ts, false, false, train_state);
		...
		hist->Draw("pe");
	c1->SaveAs(Form("BDTCorYieldplot_train%d_%dS_%ld_2.C",train_state,yield_state,ts) );
	TFile* f_out= new TFile(Form("BDTCorYield_plots_%ld.root",ts), "update");
	f_out->cd();
	hist->Write();
	f_out->Close();
