...
void _BDTFIT(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "4", const string Trig = "", int cBinLow =0, int cBinHigh = 180, double cutQVP = 0.01, double cutBDT=-1 ,Double_t params[/*sigma1S_1, alpha, n , frac, Erfmean, Erfsigma, Erfp0*/]= {}){
  Double_t etaMax= 2.4;
  Double_t etaMin = -2.4;
  ...
  TString massDIR = mainDIR +  Form("/MassDist/BDT_IDcheck/%s/DatafixtoMC/%s/%s/FitResult",ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIRp = mainDIR +  Form("/MassDist/BDT_IDcheck/%s/DatafixtoMC/%s/%s/FitResult/png", ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIRl = mainDIR + Form("/MassDist/BDT_IDcheck/%s/DatafixtoMC/%s/%s/FitResult/logY", ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIR2 = mainDIR + Form("/MassDist/BDT_IDcheck/%s/DatafixtoMC/%s/%s/WithoutFit", ts.c_str(), Trig.c_str(), MupT.Data());
  ...
  TFile* fout;
  fout = new TFile(Form("Yield/Yield_%s_DFM_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f_%s%.4f_IDcheck.root",ts.c_str(), (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(),(int) swflag, "BDT",1,"cut",cutBDT,"vp",cutQVP), "RECREATE");
...
//  argRD->add(*(works1->var("pBDT")));
  ...
  RooDataSet* reducedDS = (RooDataSet*) initialDS->reduce(RooArgSet(*(works1->var("mass"))), Form("( pt >=%f && pt <=%f) && (y >= %f && y <=%f) && (cBin>=%d && cBin<=%d) &&(pt1 >= %f) && (pt2 >= %f) && (eta1 >= %f && eta1 <= %f) && ( eta2 >= %f && eta2 <= %f) && (BDT> %f)&&(QQVtxProb > %f)", ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh, MupTCut, MupTCut, etaMin, etaMax, etaMin, etaMax, cutBDT, cutQVP));
  ...
  RooRealVar alpha("alpha", "alpha of Crystal ball", 2.0);
  RooRealVar n("n", "n of Crystal ball", 2.0);
...
//  RooAddPdf* CBG3S = new RooAddPdf("CBG3S", "Sum of 3S Crystal ball Gauss", RooArgList(*CB3S_1, *G3S), RooArgList(*frac));
  ...
  c1->SaveAs(Form("%s/MassDistribution_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f_%s%.4f.pdf",massDIR.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDT,"vp",cutQVP));
  c1->SaveAs(Form("%s/png/MassDistribution_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f_%s%.4f.png",massDIR.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDT,"vp",cutQVP));
  TCanvas* c1_tmp = (TCanvas*) c1->Clone("ylogc1");
  c1_tmp->SetLogy();
  c1_tmp->Modified();
  c1_tmp->SaveAs(Form("%s/MassDistribution_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f_%s%.4f.pdf",massDIRl.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDT,"vp",cutQVP));
