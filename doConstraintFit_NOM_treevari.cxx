...
void doConstraintFit_NOM_treevari(int step = 0){
 ...
 for( auto rp : rapPair ) {
   for( auto pp : ptPair ) {
     for( auto cp : cBinPair ){
      ...
      long xts = ts;
//    std::pair<double, RooRealVar> res = prep_bdtval(-0.0, depth);
//    cutBDTlow = res.first;
...
double INTBIN_BDTLOW = 0.1632;// =0.16;
train_state = 3;
state =3;
/* //Sealed
 if(step == 1){
