...
//  if(strcmp(aux.c_str(),"")!= 0 ) aux = "_"+aux;
  ...
  std::string name_file_output = GetFit(__FITRESLATEST, false, type, kts, train_state, sig_state, (int) ptMin, (int) ptMax, cBinLow, cBinHigh, cutBDTlow, cutBDThigh, bdtptMin, bdtptMax, cutQVP, aux);
...
// if(ts >= 1634636609) name_file_output = Form("%s/Yield/Yield_%dS_%ld_%s%s_pt_%d-%d_rap_-%d-%d_cbin_%d-%d_MupT%s_Trig_%s_SW%d_BDT%d_cut%.4f-%.4f_bdtpt_%d_%d_vp%.4f%s.root" ,workdir.Data(), train_state, kts, fitdir.c_str(), name_fitmodel.c_str(), (int) ptMin, (int) ptMax,  ylim10, ylim10, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag, (int) isBDT, cutBDTlow, cutBDThigh, bdtptMin, bdtptMax, cutQVP, aux.c_str() );
  ...
  TString massDIR_base = Form("%s/MassDist/%s/%ld%s/Cent%d-%d_Pt%d-%d_y%.1f/%d/%s", workdir.Data(), method_selection.c_str(), kts, alt.c_str(), cBinLow, cBinHigh, (int) ptMin, (int) ptMax, rapMax, sig_state, fit_param.c_str());
...
/////////////////////////////////////////////////////////////////////////////
  ...
  lt0->DrawLatex(0.42,0.80, Form("BDT #in [%.2f, %.2f]",cutBDTlow, cutBDThigh));
  lt0->DrawLatex(0.42,0.75, Form("BDT p_{T} #in [%d, %d]",bdtptMin, bdtptMax));
  lt0->DrawLatex(0.42,0.70, Form("p_{T}^{#mu} #geq %s GeV/c",MupT.Data()));
  lt0->DrawLatex(0.42,0.65, Form("%d #leq p_{T}^{#mu#mu} < %d GeV/c", (int) ptMin, (int) ptMax));
  lt0->DrawLatex(0.42,0.60, Form("Centrality %d-%d %%",(int) cBinLow/2, (int) cBinHigh/2));
  lt0->DrawLatex(0.42,0.55, Form("Trigger : %s", Trig.c_str()));
