...
void DrawNominalMassDist_v5_DataFitCompareWpar(){
...
//	plot1->GetYaxis()->SetLimits(400 *sf, 1200 * sf);
//	plot1->GetYaxis()->SetRangeUser( 400 *sf, 1200 * sf);


	gStyle->SetEndErrorSize(0);

//  setTDRStyle();
  TCanvas* c1 = new TCanvas("c1", "", 1150, 810);


  TPad* pad_mass = new TPad("pad_mass", "pad_mass", 0.00, 0.25, 0.7, 1.);
...
//  partplot->Draw();
  ...
  c1->Update();
  c1->SaveAs(Form("FitMass_NOMFITp_v5_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
  c1->SaveAs(Form("../checkout/FitMass_NOMFITp_v5_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
