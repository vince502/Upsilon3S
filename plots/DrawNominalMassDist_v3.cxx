...
//  TLegend* leg = new TLegend(0.71, 0.42, 0.95,0.7 );
  TLegend* leg = new TLegend(0.61, 0.18, 0.78,0.4 );
...
//  Ptext->Draw();
  ...
  c1->Update();
  c1->SaveAs(Form("FitMass_NOM_v3_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
  c1->SaveAs(Form("../checkout/FitMass_NOM_v3_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
  ...
  c2->SaveAs(Form("FitMass_NOMRAW_v3_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
  c2->SaveAs(Form("../checkout/FitMass_NOMRAW_v3_%ld_%dS_%d-%dGeV_%d-%dCent.pdf", ts, ab.state, ab.pl, ab.ph, ab.centl, ab.centh));
  c1->Clear();
  c2->Clear();
}
