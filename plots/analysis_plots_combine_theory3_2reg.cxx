void analysis_plots_combine_func_theory3_2reg(TFile* hf, TFile* hsys, TFile* ppsys, TFile* hdff, TFile* hdfsys, TFile* ppdfsys)
{
...
//////////////////////////////Global Uncertainties/////////////////////////////
	...
	double glb_3serr_dr = hpp_i23s_unc->GetBinContent(1);
...
//	glb_errMB_PP =quadsum(pp_lum, mb_unc);
	...
	b_pterr->SetFillColorAlpha(12, 0.0);
...
//////////////////////////////Auxiliary Drawings///////////////////////////////
	...
	TLegend* leg_pt = new TLegend(0.18, 0.45, 0.52, 0.63);
	...
	TLegend* leg_cent_theory = new TLegend(0.65, 0.45, 0.85, 0.67);
	leg_cent_theory->SetBorderSize(0);
	leg_cent_theory->SetTextFont(42);
	leg_cent_theory->SetTextSize(0.032);
...
//	g_c2s->Draw("PE");
	...
	g_c2s_sys->Draw("5");
	g_c3s_sys->Draw("5");
    draw_theory_Transport(p1_L, leg_cent_theory, kTCent, 2);
    draw_theory_Transport(p1_L, leg_cent_theory, kTCent, 3);
//    draw_theory_QTraj(p1_L,  leg_cent, kTCent, 2);
//    draw_theory_QTraj(p1_L,  leg_cent, kTCent, 3);
...
//	g_p2s->Draw("PE");
	...
	g_p2s_sys->Draw("5");
	g_p3s_sys->Draw("5");
    draw_theory_Transport(p2,leg_pt_theory,  kTPt, 2);
    draw_theory_Transport(p2,leg_pt_theory,  kTPt, 3);
//    draw_theory_QTraj(p2, leg_pt, kTPt, 2);
//    draw_theory_QTraj(p2, leg_pt, kTPt, 3);
	TLine* p2_line1 = lineone->DrawLine(0, 1, 30, 1);
	tl->SetTextSize(0.042);
	tl->SetTextAlign(11);
//	TLatex* p2_latex1 = tl->DrawLatex( 2.5, 1.15,"#it{p}_{T} < 30 GeV/c");
...
//	towrite["xcx"] = xcx;
    ...
    b_3serr->SetX1(390);
    b_3serr->SetX2(420);
    b_3serr->SetY1(1 - fabs(glb_3serr_dr));
    b_3serr->SetY2(1 + fabs(glb_3serr_dr));
    b_3serr->SetFillColorAlpha(kTeal+5, 0.8);
    b_3serr->SetLineWidth(1);
	b_3serr->SetLineColor(kGray+2);

	p1_L->SetLeftMargin(0.15);
	p2->SetLeftMargin(0.13);

	leg_cent = new TLegend(0.43,0.625, 0.65, 0.68);
	leg_cent->AddEntry(g_c23s, "Data", "pe");
	leg_cent->SetNColumns(1);
	leg_cent->SetBorderSize(0);
	leg_cent->SetTextFont(42);
	leg_cent->SetTextSize(0.030);
	leg_cent->SetFillStyle(0);
	auto leg_cent_box = new TLegend(0.43,0.625, 0.65, 0.68);
	leg_cent_box->AddEntry(g_c23s_sys, "Data", "f");
	leg_cent_box->SetNColumns(1);
	leg_cent_box->SetBorderSize(0);
	leg_cent_box->SetTextFont(42);
	leg_cent_box->SetTextSize(0.030);
	leg_cent_box->SetFillStyle(0);
	leg_cent_theory = new TLegend(0.65, 0.625, 0.92, 0.71);
	leg_cent_theory->SetNColumns(1);
	leg_cent_theory->SetBorderSize(0);
	leg_cent_theory->SetTextFont(42);
	leg_cent_theory->SetTextSize(0.030);
	leg_cent_theory->SetHeader("#lower[-0.1]{#scale[1.2]{#bf{TAMU}}}");
	TLatex* leg_cent_addtxt = new TLatex();
	leg_cent_addtxt->SetTextFont(42);
	leg_cent_addtxt->SetTextSize(0.03);

	leg_pt = new TLegend(0.37,0.625, 0.55, 0.68);
	leg_pt->AddEntry(g_p23s, "Data", "pe");
	leg_pt->SetNColumns(1);
	leg_pt->SetBorderSize(0);
	leg_pt->SetTextFont(42);
	leg_pt->SetTextSize(0.030);
	leg_pt->SetFillStyle(0);
	auto leg_pt_box = new TLegend(0.37,0.625, 0.55, 0.68);
	leg_pt_box->AddEntry(g_p23s_sys, "Data", "f");
	leg_pt_box->SetNColumns(1);
	leg_pt_box->SetBorderSize(0);
	leg_pt_box->SetTextFont(42);
	leg_pt_box->SetTextSize(0.030);
	leg_pt_box->SetFillStyle(0);
	leg_pt_theory = new TLegend(0.55, 0.625, 0.86, 0.71);
	leg_pt_theory->SetNColumns(1);
	leg_pt_theory->SetBorderSize(0);
	leg_pt_theory->SetTextFont(42);
	leg_pt_theory->SetTextSize(0.030);
	leg_pt_theory->SetHeader("#lower[-0.1]{#scale[1.2]{#bf{TAMU}}}");
	TLatex* leg_pt_addtxt = new TLatex();
	leg_pt_addtxt->SetTextFont(42);
	leg_pt_addtxt->SetTextSize(0.03);
...
//    draw_theory_QTraj(p1_L, leg_cent, kTCent, 32);
	std::cout << "Fetching double ratio" << std::endl;
    draw_theory_Transport(p1_L, leg_cent_theory,  kTCent, 32, false, true, true);
    draw_theory_Transport(p1_L, leg_cent_theory,  kTCent, 32, false, false, false);
	TList* lpc = leg_cent_theory->GetListOfPrimitives();
	TIter nextc(lpc);
	TLegendEntry *le;
	TObject* obj;
	int i = 0;
	while( (obj = nextc())){
		le = (TLegendEntry*) obj;
		if(i==1)le->SetLabel("No recombination");
		if(i==2)le->SetLabel("Total");
		i++;
	}
	...
	leg_cent->Draw();
	leg_cent_theory->Draw();
//	leg_cent_addtxt->DrawLatex(170, 1.063, "(without regeneration)");
	b_3serr->Draw("L");
...
//	g_p23s->Draw("PE");
    draw_theory_Transport(p2, leg_pt_theory, kTPt, 32, false, true, true);
    draw_theory_Transport(p2, leg_pt_theory, kTPt, 32, false, false);
	TList* lpp = leg_pt_theory->GetListOfPrimitives();
	TIter nextp(lpp);
//	TLegendEntry *le;
//	TObject* obj;
	i = 0;
...
//    draw_theory_QTraj(p2, leg_pt, kTPt, 32);
	...
	tl->SetTextSize(0.042);
	tl->SetTextAlign(11);
//	tl->DrawLatex( 2.5, label_pos_up + 0.05,"#it{p}_{T} < 30 GeV/c");
	tl->DrawLatex( 2.5, label_pos_up + 0.02 , "|#it{y}| < 2.4");
	tl->DrawLatex( 2.5,label_pos_up - 0.15 , "Cent. 0-90%");
	leg_pt_box->Draw();
	leg_pt->Draw();
	leg_pt_theory->Draw();
//	leg_pt_addtxt->DrawLatex(11.4, 1.063, "(without regeneration)");
	...
	c1->SaveAs(Form("../checkout/tscol/DR_centrality_int_%ld_v3_theory3_2reg.pdf", _TS) );
	c2->SaveAs(Form("../checkout/tscol/DR_pt_%ld_v3_theory3_2reg.pdf", _TS) );
	auto output = new TFile(Form("resultDR_%ld_v3.root", _TS), "recreate");
	dbg();
	auto Dir_vanila = output->mkdir("vanila");
	dbg();
	Dir_vanila->cd();
	c1->Write();
	c2->Write();
