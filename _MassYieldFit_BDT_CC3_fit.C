...
void _MassYieldFit_BDT_CC3_fit(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "3p5", const string Trig = "", int cBinLow =0, int cBinHigh = 180, double cutQVP = 0.01, double cutBDTlow=-1, double cutBDThigh = 1. ,Double_t params[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {},Double_t paramslow[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {},Double_t paramshigh[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {}){
  ...
  string ts = "123123";//fname.substr(pos,10);
  ...
  Int_t Nmassbins;
  long tsnum = stol(ts);
  if (massrng.find(tsnum) == massrng.end()){
  RangeLow = 8;
  RangeHigh = 14;
  Nmassbins = 120;
  }
  else{
    RangeLow = massrng[tsnum].first;
    RangeHigh = massrng[tsnum].second;
    Nmassbins = (RangeHigh - RangeLow)/0.05;
  }
  
  mf = massfitter( "/home/vince402/Upsilon3S/roodatasetFiles/OniaRooDataSet_OniaSkim_TrigS13.root", "test.root" );
  mf.Range_fit_low = RangeLow;
  mf.Range_fit_high = RangeHigh;
  Double_t MupTCut = single_LepPtCut(MupT);


  mf.list_arg	= {"mass", "pt", "y", "cBin", "pt1", "pt2", "eta1", "eta2"};
  mf.dsCut	= Form("( pt >=%f && pt <=%f) && (y >= %f && y <=%f) && (cBin>=%d && cBin<=%d) &&(pt1 >= %f) && (pt2 >= %f) && (eta1 >= %f && eta1 <= %f) && ( eta2 >= %f && eta2 <= %f)", ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh, MupTCut, MupTCut, etaMin, etaMax, etaMin, etaMax);
  mf.ws_init("dataset",mf.list_arg);
...
//  ch4_k4.setVal(params[7]);
  ...
  RooFitResult* Result = mf.works->pdf("model")->fitTo(*mf.iDS, Save(), NumCPU(2), Hesse(kTRUE), Range(RangeLow, RangeHigh), Minos(0), SumW2Error(kTRUE), Extended(kTRUE));
  mf.works->pdf("model")->plotOn(massPlot, Name("modelPlot"));
  mf.works->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal1S)), LineColor(kRed), LineStyle(kDashed), MoveToBack());
  mf.works->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal2S)), LineColor(kRed), LineStyle(kDashed), MoveToBack());
  mf.works->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal3S)), LineColor(kRed), LineStyle(kDashed), MoveToBack());
  mf.works->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Background)), LineColor(kBlue), LineStyle(kDashed));
  massPlot->SetTitle("");
  ...
  parList.Print("v");
  mf.works->pdf("model")->paramOn(legPlot, Layout(0, 0.93, 0.97), Parameters(parList));
  legPlot->getAttText()->SetTextAlign(11);
  legPlot->getAttText()->SetTextSize(0.05);

  TPaveText* Ptext = (TPaveText*) legPlot->findObject(Form("%s_paramBox", mf.works->pdf("model")->GetName()));
  Ptext->SetY1(0.01);
  Ptext->SetY2(0.95);
  Ptext->Draw();
  ...
  Double_t Yield2S = mf.works->var("nSig2S")->getVal();
  Double_t Yield2SErr = mf.works->var("nSig2S")->getError();
  Double_t Yield3S = mf.works->var("nSig3S")->getVal();
  Double_t Yield3SErr = mf.works->var("nSig3S")->getError();
  ...
  wtmp->import(*mf.works->pdf("model")); 
