...
void BDTClassApp_NOM_treevari(long ts= 9999999999, long target_ts = 9999999991, int train_state =3, int target_state = 3, int isMC = 0, bool isbbb = true, int ptLow =0, int ptHigh = 30){
  ...
  TTree* tree2;
  if(isMC==0){
  tree1 = (TTree*) input->Get("tree4");
  tree2 = (TTree*) input->Get("tree5");
  }
  if(isMC!=0){
  ...
  TFile *target;
  if (isMC ==0) target =new TFile(Form("./BDTAppliedData/BDTApp_%ld.root",target_ts),"update");
  else if (isMC==1)
  {
    if(target_state ==1 ) target =new TFile(Form("./BDTAppliedData/BDTApp_%ld_MC_1S.root",target_ts),"update");
    if(target_state ==2 ) target =new TFile(Form("./BDTAppliedData/BDTApp_%ld_MC_2S.root",target_ts),"update");
    if(target_state ==3 ) target =new TFile(Form("./BDTAppliedData/BDTApp_%ld_MC.root",target_ts),"update");
  }
  TTree* outtree;
  bool isNew = true;
...
//void BDTClassApp_NOM_treevari_(long ts= 9999999999, long target_ts = 9999999991, int train_state =3, int target_state = 3, int isMC = 0, bool isbbb = true, int ptLow =0, int ptHigh = 30){
