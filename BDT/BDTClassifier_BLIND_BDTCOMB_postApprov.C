...
bool BDTClassifier_BLIND_Function(int state , int idx , double ptLow, double ptHigh, int cBinLow, int cBinHigh, std::string opt = "", long inputts =0, string bookOpt = "", string datprep ="", string inputdataset = ""){
  //Load Library
...
//  TFile* inputDATA = new TFile(Form("%s/%s", store.Data(), ONIABDTDATAB_LATEST.c_str()),"read");
  TFile* inputDATA = new TFile(Form("%s/%s", store.Data(), inputdataset.c_str()),"read");
...
//  TTree* BkgTreeTrain2 = BkgTreeTrain_primary2->CopyTree("(mass>8.&&mass<8.6)||(mass>10.8&&mass<14)");
  ...
  TTree* BkgTreeTrain1 = BkgTreeTrain_primary1->CopyTree("(mass>8.&&mass<14)");
  TTree* BkgTreeTrain2 = BkgTreeTrain_primary2->CopyTree("(mass>8.&&mass<14)");
...
//    if(_ts - 100009000000 == 1 || true) loader->AddVariable("QQVtxProb", "Vtx prob", "F");
    ...
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("isCowboy", "Cowboy", "I");
 //   if(_ts - 100009000000 == 1 || true) loader->AddVariable("mass", "Dimuon Mass", "F");
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("log(abs(QQdca))", "QQdca", "F");
    ...
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("log(abs(ctau))", "2 dim ctau of the dimuon","F");
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("log(abs(ctau3D))", "3 dim ctau of the dimuon","F");
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("cosAlpha", "cos alpha for trajectory angle", "F");
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("cosAlpha3D", "cos alpha for trajectory angle 3D", "F");
    if(_ts - 100009000000 == 1 || true) loader->AddVariable("ctau * cosAlpha3D / QQdca", "triangluar variable", "F");
...
//Main Function
void BDTClassifier_BLIND_BDTCOMB_postApprov( long _inputts , string _bookOpt, string _datprep, bool nomonly = false, string inputdataset = ONIABDTDATAB_LATEST ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  std::vector<std::pair<int, int> >bin3spt = {{0,3}};//{/*{0,4},{4,9},{9,12},{12,15},{15,50},{12,24},{24,50},{12,20},{20,50},{12,30},{30,50},{15,30}*/};
  ...
  {
  		res = BDTClassifier_BLIND_Function(3,0,pair.first,pair.second, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
  	}
  }
  	else{ 
		res = BDTClassifier_BLIND_Function(3,0,0, 30, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//		res = BDTClassifier_BLIND_Function(3,0,4, 9, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//		res = BDTClassifier_BLIND_Function(3,0,0, 3, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//
//		res = BDTClassifier_BLIND_Function(3,0,9, 15, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//		res = BDTClassifier_BLIND_Function(3,0,3, 6, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//		res = BDTClassifier_BLIND_Function(3,0,6, 9, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
//
//		res = BDTClassifier_BLIND_Function(3,0,15,30, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep, inputdataset);
//  		if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
	}
