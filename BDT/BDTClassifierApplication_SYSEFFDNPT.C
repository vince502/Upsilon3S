...
void BDTClassifierApplication_SYSEFFDNPT(long ts= 9999999999, int train_state =2, int target_state = 2, int isMC = 0, bool isbbb = true, int ptLow =0, int ptHigh = 30, int sys_step = 1){
  ...
  TString fname;
  if(isMC){
    if(target_state==2 && sys_step ==  1) fname =Form("%s/%s", dnptdir.Data(), SYS_DNPTMC2SUP_RAW.c_str());
    if(target_state==3 && sys_step ==  1) fname =Form("%s/%s", dnptdir.Data(), SYS_DNPTMC3SUP_RAW.c_str());
    if(target_state==2 && sys_step == -1) fname =Form("%s/%s", dnptdir.Data(), SYS_DNPTMC2SDOWN_RAW.c_str());
    if(target_state==3 && sys_step == -1) fname =Form("%s/%s", dnptdir.Data(), SYS_DNPTMC3SDOWN_RAW.c_str());
  }
  else if(!isMC){
  ...
  {
    if(target_state ==2 && sys_step ==  1) target =new TFile(Form("%s/%s", dnptdir.Data(), SYS_DNPTMC2SUP.c_str()),"update");
    if(target_state ==3 && sys_step ==  1) target =new TFile(Form("%s/%s", dnptdir.Data(), SYS_DNPTMC3SUP.c_str()),"update");
    if(target_state ==2 && sys_step == -1) target =new TFile(Form("%s/%s", dnptdir.Data(), SYS_DNPTMC2SDOWN.c_str()),"update");
    if(target_state ==3 && sys_step == -1) target =new TFile(Form("%s/%s", dnptdir.Data(), SYS_DNPTMC3SDOWN.c_str()),"update");
  }
  TTree* outtree;
  bool isNew = true;
