...
//  string pbdtv = Form("prob_BDT_train_%s",ts.c_str());
  ...
  Double_t dxy1, dxy2, dz1, dz2;
  Int_t nPixWMea1, nPixWMea2, nTrkWMea1, nTrkWMea2;
...
//    }
  ...
  TFile* wf = new TFile(Form("roodatasets/OniaRooDataset_BDT%ld_OniaSkim_TrigS13_BDT_IDcheck.root",ts),"recreate");
  wf->cd();
  dataSet->Write();
  wf->Write();
}
