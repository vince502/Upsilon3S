...
void makeRooDataset_fromBDT_CENT(long ts, bool cutID, bool isMC, int train_state = 3, int cbinLow =0 , int cbinHigh =30, std::string aux_opt = "nan"){
  ...
  tree -> SetBranchAddress(Form("BDTY%dScbin%dto%dpt0to30", train_state, cbinLow, cbinHigh), &BDT, &b_BDT);
  if( cutID){
...
//  tree -> SetBranchAddress(Form("prob_BDT_train_%s",ts.c_str()), &pBDT, &b_BDT_prob);
  ...
  //////////////////  RooDataSet 
  ////////////////////////////////////////////////////////////////////////
  RooRealVar* massVar  = new RooRealVar("mass","mass",0,200,"GeV/c^{2}");
  ...
  RooRealVar* evtWeight = new RooRealVar("weight","weight", 0, 10000,"");
...
//  RooRealVar* pBDTVar  = new RooRealVar("pBDT", "BDT probabilty", 0,1, ""); 
  ...
  RooDataSet* dataSet  = new RooDataSet(Form("dataset_Y%dScbin%dto%dpt0to30", train_state, cbinLow, cbinHigh), " a dataset", *argSet);
...
//    }
  ...
  RooDataSet* WdataSet  = new RooDataSet(Form("dataset_Y%dScent%dto%dpt0to30", train_state, cbinLow, cbinHigh), " a dataset", dataSet, *dataSet->get(),0,"weight");
