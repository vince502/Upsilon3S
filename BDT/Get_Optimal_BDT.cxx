...
std::pair<double,double> Get_Optimal_BDT(long ts, double ptMin, double ptMax, double rapMin, double rapMax, int cBinLow, int cBinHigh, double cutQVP, string name_input_opt = "", string formula_significance= "S12")
{
  ...
  std::cout << Nbins << "NBINS!!!!!" << std::endl;
  int ent_1 = tree_train->GetEntries("classID==0");
  int ent_2 = tree_train->GetEntries("classID==0");
  
  TH1D* hist_res = new TH1D(Form("weighted_hist_res %ld, [%.1f,%.1f][%.1f,%.1f][%d,%d]",ts, ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh), "signifcance vs BDT", Nbins, -0.5, 1);
  TH1D* hist_res_base = new TH1D(Form("base_hist_res %ld, [%.1f,%.1f][%.1f,%.1f][%d,%d]",ts, ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh), "signifcance vs BDT", Nbins, -0.5, 1);
  ...
  int max_sig = 20000;
  int max_bkg = 20000;
  int ent_smol = min(ent_1,ent_2);
  int min_smol = min(max_sig,ent_smol);
  max_sig = min_smol;
  max_bkg = min_smol;
  ...
  for(int idx =0 ; idx <Nbins; idx++){
    ...
    if( formula_significance=="S2") {
      _signif[idx] = pass_sig/TMath::Sqrt(pass_sig+pass_bkg); 
      if ( pass_sig ==0 && pass_bkg ==0) {_signif_err[idx] =0; if(zero_bin ==0) zero_bin=idx;} 
    ...
    hist_res_base->SetBinError(idx+1, _signif_err[idx]);
  }
  Get_Optimal_BDT_HIST = hist_res_base;
  TFile* output;
  TCanvas* c1  = new TCanvas(Form("Plot %ld, [%.1f,%.1f][%.1f,%.1f][%d,%d]",ts, ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh), "signifcance vs BDT");
  output = new TFile(Form("%s/BDT/test_OPT_BDT_method2.root",workdir.Data()),"update");
  output->cd();

  //TF1 *f1 = new TF1("poly3", "[0]*x^3+[1]*x^2+[2]*x+[3]",-0.3, 0.4);
  hist_res_base->Fit("pol5","R","",-0.5,interval_score*(zero_bin)-0.5);
  hist_res->Draw();
  ...
  auto res = hist_res_base->GetFunction("pol5");
  double max_signif_bdt_fromfit = res->GetMaximumX(-0.3, 0.5); 
  double max_signif_bdt = -0.5+0.5*interval_score+interval_score*(hist_res_base->GetMaximumBin()-1);
  double max_signif_val = hist_res_base->GetMaximum();
  c1->Close();

  return std::make_pair(max_signif_bdt_fromfit, max_signif_val);
};
