...
void BDTClassifierApplication_NOM_NB(long ts= 9999999999, int train_state =2, int target_state = 2, bool isMC = 0, bool isbbb = true, int ptLow =0, int ptHigh = 30, int mcmode= 0, long ref = 10000000016){
...
//  if (whichtree !=0 && whichtree <6) {std::cout <<"Application in BLIND tree"<<whichtree<< std::endl;}
  ...
  TXMLEngine xml;
  XMLDocPointer_t xmldoc = xml.ParseFile(Form("./data/Y%dSpt%dto%d/dataset1/weights/TMVA_BDT_Classifier1_%ld_BDT.weights.xml", train_state, ptLow, ptHigh, ref));
  ...
  if(isMC){
    ...
    if(whichtree !=0){
      ...
      if(ts == 20000000001) fname =Form("%s/%s", store.Data(), "test220217_1.root" );
      if(ts == 20000000011) fname =Form("%s/%s", store.Data(), "test220217_1.root" );
      if(ts == 20000000003) fname =Form("%s/%s", store.Data(), "test220217_3.root" );
      if(ts == 20000000004) fname =Form("%s/%s", store.Data(), "test220217_4.root" );
  ...
  TString weightfile1 = Form("./data/Y%dSpt%dto%d/dataset1/weights/TMVA_BDT_Classifier1_%ld_BDT.weights.xml", train_state, ptLow, ptHigh, ref);
  TString weightfile2 = Form("./data/Y%dSpt%dto%d/dataset2/weights/TMVA_BDT_Classifier2_%ld_BDT.weights.xml", train_state, ptLow, ptHigh, ref);
