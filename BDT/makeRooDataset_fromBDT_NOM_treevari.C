...
void makeRooDataset_fromBDT_NOM_treevari(long ts, bool cutID, bool isMC, int train_state = 3, int ptLow =0 , int ptHigh =30, std::string aux_opt = "nan"){
  string smc = "";
  if (isMC) smc = "_MC";
  ...
  tree -> SetBranchAddress(Form("BDTY%dSpt%dto%d", train_state, ptLow, ptHigh), &BDT, &b_BDT);
  if( cutID){
