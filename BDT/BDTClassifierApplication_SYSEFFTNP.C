...
//  if (whichtree !=0 && whichtree <6) {std::cout <<"Application in BLIND tree"<<whichtree<< std::endl;}
  ...
  TString fname;
  if(isMC){
    if(target_state==2) fname =Form("%s/%s", tnpdir.Data(), SYS_TNPMC2S_RAW.c_str());
    if(target_state==3) fname =Form("%s/%s", tnpdir.Data(), SYS_TNPMC3S_RAW.c_str());
  }
  else if(!isMC){
  ...
  {
    if(target_state ==2 ) target =new TFile(Form("%s/%s", tnpdir.Data(), SYS_TNPMC2S.c_str()),"update");
    if(target_state ==3 ) target =new TFile(Form("%s/%s", tnpdir.Data(), SYS_TNPMC3S.c_str()),"update");
  }
  TTree* outtree;
  bool isNew = true;
