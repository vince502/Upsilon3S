...
void BDTClassifierApplication_CENT(long ts= 9999999999, int train_state =2, int target_state = 2, bool isMC = 0, bool isbbb = true, int cbinLow =0, int cbinHigh = 30, int mcmode= 0){
...
//  if (whichtree !=0 && whichtree <6) {std::cout <<"Application in BLIND tree"<<whichtree<< std::endl;}
  ...
  TXMLEngine xml;
  XMLDocPointer_t xmldoc = xml.ParseFile(Form("./data/Y%dScbin%dto%dpt0to30/dataset1/weights/TMVA_BDT_Classifier1_%ld_BDT.weights.xml", train_state, cbinLow, cbinHigh, ts));
  ...
  TString weightfile1 = Form("./data/Y%dScbin%dto%dpt0to30/dataset1/weights/TMVA_BDT_Classifier1_%ld_BDT.weights.xml", train_state, cbinLow, cbinHigh, ts);
  TString weightfile2 = Form("./data/Y%dScbin%dto%dpt0to30/dataset2/weights/TMVA_BDT_Classifier2_%ld_BDT.weights.xml", train_state, cbinLow, cbinHigh, ts);
  ...
  TBranch* bdt_branch = outtree->Branch(Form("BDTY%dScbin%dto%dpt0to30",train_state, cbinLow, cbinHigh) , &BDT, Form("BDTY%dScbin%dto%dpt0to30/D",train_state, cbinLow, cbinHigh));
