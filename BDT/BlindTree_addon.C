...
void BlindTree_addon(){
  TFile* fo = new TFile(Form("%s/%s", store.Data(), ONIABDTDATA_DEPRE_LATEST.c_str()),"READ");
  TFile* fn = new TFile(Form("%s/OutputSkim_isMC0_v211025_m68_ForBLIND.root", store.Data()) ,"recreate");
...
//  TFile* fn = new TFile(Form("%s/%s", hfdir.Data(), SYS_HFUPDATA.c_str()) ,"recreate");
  TTree* t0_pre = (TTree*) fo->Get("tree");
  TTree* t0 = (TTree*) t0_pre->CopyTree("mass<8");
