...
std::pair<double, double> getEffhist_SYSNODPT(float pl, float ph, float yl, float yh, int cl, int ch, bool istnp, bool wei, long ts, double bdt_low, double bdt_high, int train_state =3,int state =3, double vcut =0.00, std::string what = ""){
  double ml, mh;
  auto mp = mass_rng(state);
  ml = mp.first;
  mh = mp.second;

  string fname =Form("%s/BDT/EffCalc/noDNDPT/mc_eff_BDT_%dS_%ld_bdt_%.4f-%.4f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_vp_%.4f_isTnP%d_isPtWeight%d_ID", workdir.Data(), state, ts, bdt_low, bdt_high, pl, ph, yl, yh, 3.5, ml, mh, cl, ch, vcut, istnp, wei);
  if(ts >= 9999999990) fname =Form("%s/BDT/EffCalc/noDNDPT/mc_eff_BDT_%dS_train%dS_%ld_bdt_%.4f-%.4f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_vp_%.4f_isTnP%d_isPtWeight%d_ID_SYSNODPT", workdir.Data(), state, train_state, ts, bdt_low, bdt_high, pl, ph, yl, yh, 3.5, ml, mh, cl, ch,vcut, istnp, wei);
...
std::pair<double, double> getEffhist_SYSNODPT(float pl, float ph, float yl, float yh, int cl, int ch, bool istnp, bool wei, int sw, int num_trig, int state =3, std::string what = "UP"){
  double ml, mh;
  auto mp = mass_rng(state);
  ml = mp.first;
  mh = mp.second;
  ...
  string fname =Form("%s/BDT/EffCalc/noDNDPT/mc_eff_%dS_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_isTnP%d_isPtWeight%d_TrigSel%s_SYSNODPT", workdir.Data(), state, pl, ph, yl, yh, 3.5, ml, mh, cl, ch, istnp, wei, ftrigSel.c_str());
...
std::pair<double, double> openEffhist_SYSNODPT(float _pl, float _ph, float _yl, float _yh, int _cl, int _ch, bool _istnp, bool wei, bool _sw, int kTrigSel, long _ts, double _bdt_tsl, double _bdt_tsh, int train_state =3, int state =3 , double _vcut = 0.00, std::string what = "", bool get_num = false){
  std::pair<double, double> gen = getEffhist_SYSNODPT( _pl,  _ph, _yl, _yh, _cl, _ch, _istnp, wei, _sw, kTrigSel, state, what);
  std::pair<double, double> reco = getEffhist_SYSNODPT( _pl,  _ph, _yl, _yh, _cl, _ch, _istnp, wei, _ts, _bdt_tsl, _bdt_tsh, train_state, state, _vcut, what);
  ...
  std::cout << Form( "[DIAGNOSIS] noDNDPT NUMGEN, NUMREO= (%.1f, %.1f)", gen.first, reco.first) << std::endl;
