...
void BDTClassifierApplication_SYSHF(long ts= 9999999998, int train_state =2, int target_state = 2, int isMC = 0, bool isbbb = true, int ptLow =0, int ptHigh = 30){
  std::string info_blind;
  std::string systype;
  std::string fileName;
  if ( ts == 9999999998){ systype = "SYSHFUP"; fileName = SYS_HFUPDATA;}
  if ( ts == 9999999997){ systype = "SYSHFDOWN"; fileName = SYS_HFDOWNDATA;}
  if( !isbbb) info_blind = info_BDT(9999999999)[2];
  if( isbbb ) info_blind = info_BDT(9999999999)[4];
  ...
  if(isMC){
    ...
    if(whichtree !=0) fname =Form("%s/%s", hfdir.Data(), fileName.c_str() );
    if(whichtree ==0) fname =Form("%s/%s", hfdir.Data(), ONIABDTDATA_LATEST.c_str() );
  }
  ...
  TFile *target;
  if (isMC ==0) target =new TFile(Form("./BDTAppliedData/BDTApp_%ld_%s.root",ts, systype.c_str()),"update");
  else if (isMC==1)
  {
