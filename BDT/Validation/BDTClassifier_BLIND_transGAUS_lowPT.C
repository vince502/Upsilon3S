...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_transGAUS_lowPT.C >> ../.past_source/_BDTClassifier_BLIND_transGAUS_lowPT_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//Main Function
void BDTClassifier_BLIND_transGAUS_lowPT( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
