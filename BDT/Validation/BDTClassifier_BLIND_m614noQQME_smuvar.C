...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_m614noQQME_smuvar.C >> ../.past_source/_BDTClassifier_BLIND_m614noQQME_smuvar_%ld.old",(long) tstamp));
  ofstream log;
...
//  TFile* inputDATA = new TFile(Form("%s/%s", hfdir.Data(), SYS_HFDOWNDATA.c_str()),"read");
  ...
  TTree* BkgTreeTest1 = BkgTreeTest_primary1->CopyTree("(mass>6.&&mass<8.6)||(mass>10.8&&mass<14)");
...
//Main Function
void BDTClassifier_BLIND_m614noQQME_smuvar( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_m614noQQME_smuvar_%ld.old",(long) _real_time));
//  }
