...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_BDTCENT.C >> ../.past_source/_BDTClassifier_BLIND_BDTCENT_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//	datprep = datprep+ Form(":nTrain_Signal=%d:nTest_Signal=%d", nBkg_Smpl*2, -1 );
...
//Main Function
void BDTClassifier_BLIND_BDTCENT( long _inputts , string _bookOpt, string _datprep){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	res = BDTClassifier_BLIND_Function(3,0,0,30, 40,60, "", _inputts, _bookOpt, _datprep);
  	res = BDTClassifier_BLIND_Function(2,0,0,30, 40,60, "continue", _inputts, _bookOpt, _datprep);
