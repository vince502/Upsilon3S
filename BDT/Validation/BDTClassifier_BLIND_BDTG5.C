...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_BDTG5.C >> ../.past_source/_BDTClassifier_BLIND_BDTG5_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//    loader->AddSpectator("QQVtxProb", "Vtx prob", "F");
  ...
  factory1->BookMethod( loader1, TMVA::Types::kBDT, TString::Format("BDT"  ),
  	"!H:!V:NTrees=400:MaxDepth=4:MinNodeSize=5%:BoostType=Grad:UseBaggedBoost:SeparationType=CrossEntropy:PruneMethod=CostComplexity:PruneStrength=1:PruningValFraction=0.3:UseRandomisedTrees=False:BaggedSampleFraction=0.4:nCuts=100:UseFisherCuts=True:CreateMVAPdfs:DoBoostMonitor:VarTransform=G+D+G+D+G");//");
...
//Main Function
void BDTClassifier_BLIND_BDTG5( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_BDTG5_%ld.old",(long) _real_time));
//  }
