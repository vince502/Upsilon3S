...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_nS.C >> ../.past_source/_BDTClassifier_BLIND_nS_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//  TFile* inputDATA = new TFile(Form("%s/%s", hfdir.Data(), SYS_HFDOWNDATA.c_str()),"read");
  ...
  if(state ==3) inputMC= new TFile(Form("%s/%s", store.Data(), "OoutputSkim_isMC1_nS_v211026.root"),"read");
...
//    loader->AddSpectator("QQVtxProb", "Vtx prob", "F");
    ...
    loader1->PrepareTrainingAndTestTree( cut1, cut2, "SplitSeed=100:SplitMode=Random:NormMode=NumEvents:nTrain_Signal=200000:!V");
    loader2->PrepareTrainingAndTestTree( cut1, cut2, "SplitSeed=100:SplitMode=Random:NormMode=NumEvents:nTrain_Signal=200000:!V");
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	res = BDTClassifier_BLIND_Function(3,0,0,30, 0,181, "");
//  	res = BDTClassifier_BLIND_Function(2,0,0,30, 0,181, "continue");
//  	res = BDTClassifier_BLIND_Function(3,0,0,30, 0,181, "continue");
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_nS_%ld.old",(long) _real_time));
//  }
