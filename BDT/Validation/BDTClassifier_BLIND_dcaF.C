...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_dcaF.C >> ../.past_source/_BDTClassifier_BLIND_dcaF_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_dcaF_%ld.old",(long) _real_time));
//  }
