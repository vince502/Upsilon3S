...
bool BDTClassifier_BLIND_Function(int state , int idx , double ptLow, double ptHigh, int cBinLow, int cBinHigh, std::string opt = "", long inputts =0, string bookOpt = "", string datprep =""){
  ...
  if(strcmp(opt.c_str(),"NOMINAL")==0) _ts = (long) 8300000007;
  std::system(Form("cat BDTClassifier_BLIND_BDT_LOWPT2.C >> ./.past_source/_BDTClassifier_BLIND_BDT_LOWPT2_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//Main Function
void BDTClassifier_BLIND_BDT_LOWPT2( long _inputts , string _bookOpt, string _datprep){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
  ...
  std::vector<std::pair<int, int> >bin2spt = {{0,30},{0, 4},{4, 9},{9, 12},{12,30},{12, 50}};//{/*{0,2},{2,4},{4,6},{6,9},{9,12},{12,15},{15,20},{12,20},{12,24},{20,50},{24,50},{30,50}*/{12,50}, {15, 50}};
  std::vector<std::pair<int, int> >bin3spt = {{0,30},{0, 4},{4, 9},{9, 12},{12,30},{12, 50}};//{/*{0,4},{4,9},{9,12},{12,15},{15,50},{12,24},{24,50},{12,20},{20,50},{12,30},{30,50},{15,30}*/};
...
//  	if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
  	res = BDTClassifier_BLIND_Function(3,0,0, 30, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep);
  	if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
  	res = BDTClassifier_BLIND_Function(2,0,0, 30, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep);
  	if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
  	res = BDTClassifier_BLIND_Function(3,0,0, 4, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep);
  	if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier__BDT%ld.old",(long) _real_time));
...
//  {
//  	res = BDTClassifier_BLIND_Function(2,0,pair.first,pair.second, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep);
//  	if(!res)std::system(Form("rm ./.past_source/_BDT_Blind_Classifier_BDT_%ld.old",(long) _real_time));
//  }
//  for( auto pair : bin1spt) 
//  {
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL", _inputts, _bookOpt, _datprep);
