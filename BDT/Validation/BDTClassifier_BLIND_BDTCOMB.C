...
//  TFile* inputDATA = new TFile(Form("%s/%s", hfdir.Data(), SYS_HFDOWNDATA.c_str()),"read");
  ...
  inputMC2S= new TFile(Form("%s/%s", store.Data(), ONIABDTMC_LATEST.c_str()),"read");
  ...
  TTree* SigTree3S =(TTree*) inputMC2S->Get("tree");
...
//    loader->AddSpectator("QQVtxProb", "Vtx prob", "F");
    ...
    Double_t signalWeight1     = 1;// 1.4e-6 ;
    Double_t signalWeight2     = 1;// 1.4e-6 ;
    Double_t backgroundWeight = 1; //1e-3;
    loader1->AddSignalTree     (SigTree2S, signalWeight1);
    loader1->AddSignalTree     (SigTree3S, signalWeight2);
    loader2->AddSignalTree     (SigTree2S, signalWeight1);
    loader2->AddSignalTree     (SigTree3S, signalWeight2);
...
//Main Function
void BDTClassifier_BLIND_BDTCOMB( long _inputts , string _bookOpt, string _datprep){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
