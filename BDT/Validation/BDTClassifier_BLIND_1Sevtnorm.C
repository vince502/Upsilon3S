...
bool BDTClassifier_BLIND_Function(int state , int idx , double ptLow, double ptHigh, int cBinLow, int cBinHigh, std::string opt = ""){
  ...
  std::system(Form("cat BDTClassifier_BLIND_1Sevtnorm.C >> ../.past_source/_BDTClassifier_BLIND_%ld.old",(long) tstamp));
  ofstream log;
...
//  log << tstamp;
 ...
 // TFile* inputDATA = new TFile(Form("%s/%s", hfdir.Data(), SYS_HFDOWNDATA.c_str()),"read");
  TFile* inputMC   ;
  if(state ==1) inputMC= new TFile(Form("%s/%s", cmsstore.Data(), ONIABDTMC1S_3SEVTMATCH.c_str()),"read");
...
//Main Function
void BDTClassifier_BLIND_1Sevtnorm( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_1Sevtnorm_%ld.old",(long) _real_time));
//  }
