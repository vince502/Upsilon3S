...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_BDT16.C >> ../.past_source/_BDTClassifier_BLIND_BDT16_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//    loader->AddSpectator("QQVtxProb", "Vtx prob", "F");
    ...
    loader1->PrepareTrainingAndTestTree( cut1, cut2, "SplitSeed=100:nTrain_Signal=200000:SplitMode=Alternate:NormMode=None:!V");
    loader2->PrepareTrainingAndTestTree( cut1, cut2, "SplitSeed=100:nTrain_Signal=200000:SplitMode=Alternate:NormMode=None:!V");


  //Book Training BDT Method
  factory1->BookMethod( loader1, TMVA::Types::kBDT, TString::Format("BDT"  ),
  	"!H:!V:NTrees=500:MaxDepth=3:MinNodeSize=5%:BoostType=Grad:AdaBoostBeta=0.6:UseBaggedBoost:SeparationType=GiniIndex:PruneMethod=CostComplexity:PruneStrength=1:PruningValFraction=0.3:UseRandomisedTrees=False:BaggedSampleFraction=0.4:nCuts=-1:CreateMVAPdfs:DoBoostMonitor");//");
...
//Main Function
void BDTClassifier_BLIND_BDT16( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_BDT16_%ld.old",(long) _real_time));
//  }
