...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_m614noQQME_FAKE.C >> ../.past_source/_BDTClassifier_BLIND_m614noQQME_FAKE_%ld.old",(long) tstamp));
  ofstream log;
...
//  TFile* inputDATA = new TFile(Form("%s/%s", store.Data(), ONIABDTDATAB_LATEST.c_str()),"read");
  TFile* inputDATA = new TFile(Form("%s/%s", store.Data(), ONIABDTMC_LATEST.c_str()),"read");
  TFile* inputMC   ;
...
// TMVA::DataLoader *loader2 = new TMVA::DataLoader(Form("data/Y%dSpt%dto%d/dataset2", state, (int) ptLow, (int) ptHigh));
  ...
  TTree* BkgTreeTest_primary1 =(TTree*) inputDATA->Get(Form("tree"));
//  TTree* BkgTreeTest_primary2 =(TTree*) inputDATA->Get(Form("tree2"));
//  TTree* BkgTreeTest1 = BkgTreeTest_primary1->CopyTree();
//  TTree* BkgTreeTest2 = BkgTreeTest_primary2->CopyTree("(mass>6.&&mass<8.6)||(mass>10.8&&mass<14)");

//  BkgTreeTest1->SetName("tree3");
//  BkgTreeTest2->SetName("tree2");
//  TTree* BkgTreeTrain_primary1 =(TTree*) inputDATA->Get(Form("tree"));
...
//  TTree* BkgTreeTrain1 = BkgTreeTrain_primary1->CopyTree("(mass>6.&&mass<8.6)||(mass>10.8&&mass<14)");
//  TTree* BkgTreeTrain2 = BkgTreeTrain_primary2->CopyTree("(mass>6.&&mass<8.6)||(mass>10.8&&mass<14)");
...
//    loader1->AddBackgroundTree (BkgTreeTrain1, backgroundWeight, "Train");
    loader1->AddBackgroundTree (BkgTreeTest_primary1, backgroundWeight);
...
//Main Function
void BDTClassifier_BLIND_m614noQQME_FAKE( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(3,0,0,30, 0,181, "continue");
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_m614noQQME_FAKE_%ld.old",(long) _real_time));
//  }
