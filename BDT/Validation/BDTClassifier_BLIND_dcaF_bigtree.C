...
//  if(strcmp(opt.c_str(),"NOMINAL2")==0) _ts = (long) 99999999992;
  std::system(Form("cat BDTClassifier_BLIND_dcaF_bigtree.C >> ../.past_source/_BDTClassifier_BLIND_dcaF_%ld.old",(long) tstamp));
  ofstream log, log2;
...
//    loader->AddSpectator("QQVtxProb", "Vtx prob", "F");
  ...
  factory1->BookMethod( loader1, TMVA::Types::kBDT, TString::Format("BDT"  ),
  	"!H:!V:NTrees=10000:MaxDepth=20:MinNodeSize=1%:BoostType=AdaBoost:AdaBoostBeta=0.7:UseBaggedBoost:SeparationType=GiniIndex:PruneMethod=CostComplexity:PruneStrength=1:PruningValFraction=0.3:UseRandomisedTrees=False:BaggedSampleFraction=0.4:nCuts=4000:CreateMVAPdfs");//:VarTransform=Gauss");
...
//Main Function
void BDTClassifier_BLIND_dcaF_bigtree( ){
  ofstream log;
  //log.open("BDT_description.log", std::ios_base::out|std::ios_base::app);
...
//  	res = BDTClassifier_BLIND_Function(1,0,pair.first,pair.second, 0,181, "NOMINAL");
  	...
  	if(!res)std::system(Form("rm ../.past_source/_BDT_Blind_Classifier_dcaF_bigtree_%ld.old",(long) _real_time));
//  }
