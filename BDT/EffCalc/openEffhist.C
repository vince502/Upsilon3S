...
if(what=="reco"){
  ...
  double NumGen = rechist->GetBinContent(1);
  return NumGen;
  ...
  string fname =Form("mc_eff_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_isTnP%d_isPtWeight%d_TrigSel%s", pl, ph, yl, yh, 3.5, 9.0, 11.0, cl, ch, istnp, wei, ftrigSel.c_str());
