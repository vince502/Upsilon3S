...
std::pair<double, double> getEffhist(string what,float pl, float ph, float yl, float yh, int cl, int ch, bool istnp, bool wei, int opt1, double opt2, double opt22, int opt3){

TString histName;

if(what=="reco"){
  string fname =Form("/home/vince402/Upsilon3S/BDT/EffCalc/mc_eff_BDT_%d_bdt_%.3f-%.3f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_isTnP%d_isPtWeight%d_ID", opt1, opt2, opt22, pl, ph, yl, yh, 3.5, 9.0, 11.0, cl, ch, istnp, wei);
...
else return std::make_pair(10000.0, 0.0);

};

std::pair<double, double> openEffhist(float _pl, float _ph, float _yl, float _yh, int _cl, int _ch, bool _istnp, bool wei, bool _sw, int kTrigSel, int _ts, double _bdt_tsl, double _bdt_tsh ){
  std::pair<double, double> gen = getEffhist("gen", _pl,  _ph, _yl, _yh, _cl, _ch, _istnp, wei, (int) _sw, 0.,0., kTrigSel);
  std::pair<double, double> reco = getEffhist("reco", _pl,  _ph, _yl, _yh, _cl, _ch, _istnp, wei, _ts, _bdt_tsl, _bdt_tsh, 0);
  ...
  std::cout << Form("Efficiency of BDT %d, ", _ts) << Form("Cut %f-%f : ", _bdt_tsl, _bdt_tsh) << BDTratio << ", " << BDTratioErr<< std::endl;
