...
void MassYieldSingleStateMCFit( struct Y1Sfitvar *Y1S ,long ts, const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "4", const string Trig = "",int cBinLow=0, int cBinHigh = 180, int state = 1, bool fixvar = false, bool swflag = false, double cutQVP= 0.01, double bdtlow = -1., double bdthigh =1. ){
  ...
  TFile* fout;
  fout = new TFile(Form("Yield/Yield_CB3_%dS_pt_%d-%d_rap_%d-%d_noWeight_MupT%s_%s_BDT_%.4f-%.4f_vp_%.4f_MC_%d.root",(int) state ,(int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), MupT.Data(), Trig.c_str(), bdtlow, bdthigh, cutQVP,(int) fixvar), "RECREATE");
...
//  RooFormulaVar mean3S("mean3S", "mean1S*mratio3", RooArgSet(mean1S, mratio3));
  ...
  if(fixvar == false) sigma3S_1  = new RooRealVar("sigma3S_1", "sigma3 of 1S", 0.17, 0.11, 0.30);
...
//std::cout << "///////////////////////////////////////////////////////// test code ////////////////////////////////////////////////////////////" << std::endl;
  ...
  RooRealVar* x3S = new RooRealVar("x3S", "sigma ratio", 0.84, 0, 1);
  RooRealVar* x3S_2 = new RooRealVar("x3S_2", "sigma ratio", 0.64, 0.1, 1);
  works1->import(*x1S);
  works1->import(*x3S);
...
//  RooFormulaVar sigma3S_2("sigma1S_2", "@0*@1", RooArgList(sigma1S_2, mratio3));
  ...
  if ( state == 1 || fixvar == false ){
     alpha = new RooRealVar("alpha", "alpha of Crystal ball", 1.2, 0.2, 3.0);
     n = new RooRealVar("n", "n of Crystal ball", 1, 0.7, 3.0);
     frac = new RooRealVar("frac", "CB fraction", 0.4, 0.05, 0.80);
     frac2 = new RooRealVar("frac2", "CB fraction 2", 0.3, 0.10, 0.80);
   }
...
//  RooCBShape* CB3S_1 = new RooCBShape("CB3S_1", "3S Crystal ball function1", *(works1->var("mass")), mean3S, sigma3S_1, alpha, n);
  ...
  RooCBShape* CB3S_3 = new RooCBShape("CB3S_3", "3S Crystal ball function3", *(works1->var("mass")), mean3S, *sigma3S_3, *alpha, *n);
...
//  RooGaussian* G3S = new RooGaussian("G3S", "3S Gaussian", *(works1->var("mass")), mean3S, sigma3S_2);
 ...
 // RooAddPdf* twoBW3S = new RooAddPdf("twoBW3S", "Sum of 3S Breit-Wigner", RooArgList(*BW3S_1, *BW3S_2), RooArgList(*frac));
//  RooAddPdf* CBBW3S = new RooAddPdf("CBBW3S", "Sum of 3S Crystal ball and Breit-Wigner", RooArgList(*CB3S_1, *CB3S_2, *BW3S_2), RooArgList(*frac, *frac2));
  RooAddPdf* twoCB1S = new RooAddPdf("twoCB1S", "Sum of 1S Crystal ball", RooArgList(*CB1S_1, *CB1S_2), RooArgList(*frac));
  RooAddPdf* threeCB3S = new RooAddPdf("threeCB3S", "Sum of 3S Crystal ball", RooArgList(*CB3S_1, *CB3S_2, *CB3S_3), RooArgList(*frac, *frac2));
...
//  Signal2S = (RooGenericPdf*) twoCB2S;
  Signal3S = (RooGenericPdf*) threeCB3S;
...
//  works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal2S)), LineColor(kRed), LineStyle(kDashed), MoveToBack());
  ...
  //if( state ==3 ) {works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal3S)), LineColor(kRed), LineStyle(kDashed), MoveToBack()); works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*CB3S_1)), LineColor(kGreen), LineStyle(kSolid), MoveToBack()); works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*BW3S_2)), LineColor(kMagenta), LineStyle(kDashDotted), MoveToBack());}
  if( state ==3 ) {works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*Signal3S)), LineColor(kRed), LineStyle(kDashed), MoveToBack()); works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*CB3S_1)), LineColor(kGreen), LineStyle(kSolid), MoveToBack()); works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*CB3S_2)), LineColor(kRed), LineStyle(kSolid), MoveToBack()); works1->pdf("model")->plotOn(massPlot, Components(RooArgSet(*CB3S_3)), LineColor(kMagenta), LineStyle(kDashDotted), MoveToBack());}
...
//  TF1* Sgnfc3S;
  ...
  if( state ==3 )SgnfcNS = works1->pdf("threeCB3S")->asTF(*(works1->var("mass")));
...
//  }
  ...
  c1->SaveAs(Form("%s/CB3_MassDistribution_%dS_pt_%d-%d_rap_%d-%d_%dbin_noWeight_MupT%s_%s_BDT_%.4f-%.4f_vp_%.4f_MC_fix%d.pdf",massDIR.Data(), state,(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, MupT.Data(), Trig.c_str(), bdtlow, bdthigh, cutQVP,(int) fixvar));
