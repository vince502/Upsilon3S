...
void SkimTree_Event_forCent_DataMC(int nevt=-1, bool isMC = true, int kTrigSel = kTrigUps, int hiHFBinEdge = 0, int PDtype = 1) 
{
...
//  mytree->SetBranchAddress("rpAng", rpAng, &b_rpAng);
  ...
  TFile* newfile;
  newfile = new TFile(Form("skimmedFiles/OniaFlowSkim_%sTrig_%sPD_isMC%d_forCent.root",fTrigName[trigIndx].Data(),fPD.Data(),isMC),"recreate");
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(!(( JP_Reco->M() >= 8.8) && ( JP_Reco->M() <=10.1)) ) continue;
