...
void getEfficiency_SYSNODPT(
  ...
  TString histName = Form("%dS_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_isTnP%d_isPtWeight%d_TrigSel%s_SYSNODPT",state, ptLow,ptHigh,yLow,yHigh,muPtCut,massLow,massHigh,cLow,cHigh,isTnP,isPtWeight,ftrigSel.c_str());
...
//  heff->SetName(Form("mc_eff_vs_pt_TnP%d_dNdPt%d_Cent%d%d",isTnP, isPtWeight, cLow, cHigh));
  ...
  if(isBDT){ outFileName = Form("/home/vince402/Upsilon3S/BDT/EffCalc/noDNDPT/mc_eff_%s.root",histName.Data());}
