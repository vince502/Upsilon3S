...
void getEfficiencyBDT_SYSDPTDOWN(
...
//  else if(!isSwitch) ftrigSel += Form("_%s",fTrigName[kTrigSel_].Data());
  ...
  if( state ==2){
    inputMC = Form("%s/%s", dnptdir.Data(), SYS_DNPTMC2SDOWN.c_str());
  }
  if( state ==3){
    inputMC = Form("%s/%s", dnptdir.Data(), SYS_DNPTMC3SDOWN.c_str());
  }
...
//  double pt_weight = 1;
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    if(ts >= 9999999990) histName = Form("BDT_%dS_train%dS_%ld_bdt_%.4f-%.4f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_vp_%.4f_isTnP%d_isPtWeight%d_ID_SYSPTDOWN", state, train_state, ts, bdt_tsl, bdt_tsh,ptLow,ptHigh,yLow,yHigh,muPtCut,massLow,massHigh,cLow,cHigh,vcut,isTnP,isPtWeight);
    }
    double ptW =1;
    weight = weight ;
