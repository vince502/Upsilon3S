...
//  double xmax = ptHigh;
  ...
  //input files
  TString inputMC =  "/home/vince402/shared/OniaTree/MC/Oniatree_Y3SMC_v210618_pdgfix.root";
  if(state ==1) inputMC = "/home/vince402/shared/OniaTree/MC/OniaTree_Ups1SMC_HydjetDrumMB_5p02TeV_v210621.root";
  if(state ==2) inputMC = "/home/vince402/shared/OniaTree/MC/OniaTree_Ups2SMC_HydjetDrumMB_5p02TeV_v210628.root";
  ...
  f1_UN = (TF1*) f1->Clone();
  f1_DN = (TF1*) f1->Clone();
  f1_up->SetParameters(f1->GetParameter(0) + f1->GetParError(0),f1->GetParameter(1) + f1->GetParError(1));    f1_down->SetParameters(f1->GetParameter(0) - f1->GetParError(0),f1->GetParameter(1) - f1->GetParError(1));
...
//  TH1D* hreco = new TH1D(Form("hreco"),"hreco",1,xmin,xmax);
  TH2D* hgen = new TH2D(Form("hgen"),"hgen",50,0,50,36,0,180);
  TH2D* hgen_NO = new TH2D(Form("hgen_NO"),"hgen_NO",50,0,50,36,0,180);
  TH2D* hgen_NU = new TH2D(Form("hgen_NU"),"hgen_NU",50,0,50,36,0,180);
  TH2D* hgen_ND = new TH2D(Form("hgen_ND"),"hgen_ND",50,0,50,36,0,180);
  TH2D* hgen_UN = new TH2D(Form("hgen_UN"),"hgen_UN",50,0,50,36,0,180);
  TH2D* hgen_DN = new TH2D(Form("hgen_DN"),"hgen_DN",50,0,50,36,0,180);
//  TH3D* hgen_bin = new TH1D(Form("hgen_bin"),"hgen",(int) (xmax-xmin),xmin,xmax);
...
//  hgen_bin->SetTitle("Gen bin");
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    mytree->GetEntry(iev);
    if(Centrality >= 181 || Centrality < 0) continue;
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
      ...
      if(JP_Gen->Pt()> 50 || JP_Gen->Pt()<0) continue;
	  ...
	  int cbinfill = (Centrality ==180) ? 179 : Centrality;
      hgen->	Fill(JP_Gen->Pt(), cbinfill, weight*pt_weight);
      hgen_NO->	Fill(JP_Gen->Pt(), cbinfill, weight);
      hgen_NU->	Fill(JP_Gen->Pt(), cbinfill, weight*pt_Weight_NU);
      hgen_ND->	Fill(JP_Gen->Pt(), cbinfill, weight*pt_Weight_ND);
      hgen_UN->	Fill(JP_Gen->Pt(), cbinfill, weight*pt_Weight_UN);
      hgen_DN->	Fill(JP_Gen->Pt(), cbinfill, weight*pt_Weight_DN);
...
//  hreco->Draw();
  ...
  //GEN
  TCanvas * cgen = new TCanvas("cgen","creco",0,0,400,400);
  cgen->cd();
  hgen->Draw();
...
//  TString outFileName = Form("mc_eff_%s.root",histName.Data());
  string outFileName = GetEffDen(__FITRESLATEST, state, massLow, massHigh);
