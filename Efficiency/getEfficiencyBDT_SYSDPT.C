...
//  else if(!isSwitch) ftrigSel += Form("_%s",fTrigName[kTrigSel_].Data());
  ...
  TString histName = Form("BDT_%dS_%ld_bdt_%.4f-%.4f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_isTnP%d_isPtWeight%d", state, ts, bdt_tsl, bdt_tsh, ptLow,ptHigh,yLow,yHigh,muPtCut,massLow,massHigh,cLow,cHigh,isTnP,isPtWeight);
   histName = histName + "_SYSDPT";
  TH1D* hreco = new TH1D(Form("hreco"),"hreco",1,xmin,xmax);
  TH1D* hreco_NU = new TH1D(Form("hreco_NU"),"hreco",1,xmin,xmax);
  TH1D* hreco_ND = new TH1D(Form("hreco_ND"),"hreco",1,xmin,xmax);
  TH1D* hreco_UN = new TH1D(Form("hreco_UN"),"hreco",1,xmin,xmax);
  TH1D* hreco_DN = new TH1D(Form("hreco_DN"),"hreco",1,xmin,xmax);
...
//  double tnp_weight = 1;
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    if(ts >= 9999999990) histName = Form("BDT_%dS_train%dS_%ld_bdt_%.4f-%.4f_pt%.1f_%.1f_y%.1f_%.1f_SiMuPt%.1f_mass%.1f_%.1f_cent%d_%d_vp_%.4f_isTnP%d_isPtWeight%d_ID_SYSDPT", state, train_state, ts, bdt_tsl, bdt_tsh,ptLow,ptHigh,yLow,yHigh,muPtCut,massLow,massHigh,cLow,cHigh,vcut,isTnP,isPtWeight);
    }
    double ptW =1;
    weight = weight ;
    pt_Weight_NN = f1->Eval(pt);
    pt_Weight_UP = f1_up->Eval(pt);
    pt_Weight_NU = f1_NU->Eval(pt);
    pt_Weight_ND = f1_ND->Eval(pt);
    pt_Weight_UN = f1_UN->Eval(pt);
    pt_Weight_DN = f1_DN->Eval(pt);
    
    hreco->Fill(pt,(weight/pt_Weight_UP)*pt_Weight_NN);
    hreco_NU->Fill(pt,(weight/pt_Weight_UP)*pt_Weight_NU);
    hreco_ND->Fill(pt,(weight/pt_Weight_UP)*pt_Weight_ND);
    hreco_UN->Fill(pt,(weight/pt_Weight_UP)*pt_Weight_UN);
    hreco_DN->Fill(pt,(weight/pt_Weight_UP)*pt_Weight_DN);
...
//  heff->SetName(Form("mc_eff_vs_pt_TnP%d_dNdPt%d_Cent%d%d",isTnP, isPtWeight, cLow, cHigh));
  TString outFileName = Form("/home/vince402/Upsilon3S/BDT/EffCalc/DNDPT/mc_eff_%s.root",histName.Data());
