...
void MassYieldFit_v2(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "4", const string Trig = "", int cBinLow =0, int cBinHigh = 180, Double_t params[/*sigma1S_1, alpha, n , frac, Erfmean, Erfsigma, Erfp0*/]= {}){
  Double_t etaMax= 2.4;
  Double_t etaMin = -2.4;
  ...
  TString massDIR = mainDIR +  Form("/MassDist/FreeFit/%s/%s/FitResult",Trig.c_str(),MupT.Data());
  TString massDIRp = mainDIR +  Form("/MassDist/FreeFit/%s/%s/FitResult/png",Trig.c_str(),MupT.Data());
  TString massDIRl = mainDIR + Form("/MassDist/FreeFit/%s/%s/FitResult/logY",Trig.c_str(),MupT.Data());
  TString massDIR2 = mainDIR + Form("/MassDist/FreeFit/%s/%s/WithoutFit",Trig.c_str(),MupT.Data());
  ...
  if(dirpM) gSystem->FreeDirectory(dirpM2);
  ...
  TFile* fout;
//  fout = new TFile(Form("Yield/Yield_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%d_%s%d.root", (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag, "", 0,"",0,"",0), "RECREATE");
  ...
  RooFitResult* Result = works1->pdf("model")->fitTo(*reducedDS, Save(), NumCPU(2),PrefitDataFraction(0.1), Hesse(kTRUE), Range(RangeLow, RangeHigh), Minos(0), SumW2Error(kTRUE), Extended(kTRUE));
  ...
  c1->SaveAs(Form("%s/MassDistribution_v2_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%d_%s%d.pdf",massDIR.Data(),(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"",0,"",0,"",0));
  c1->SaveAs(Form("%s/png/MassDistribution_v2_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%d_%s%d.png",massDIR.Data(),(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"",0,"",0,"",0));
  TCanvas* c1_tmp = (TCanvas*) c1->Clone("ylogc1");
  c1_tmp->SetLogy();
  c1_tmp->Modified();
  c1_tmp->SaveAs(Form("%s/MassDistribution_v2_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%d_%s%d.pdf",massDIRl.Data(),(int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"",0,"",0,"",0));
