...
void DrawRatioHistPlot(TH1* h1, TH1* h2, string legheader, string legname1, string legname2, double posx1, double posx2, double posy1, double posy2, double rymin, double rymax, bool islog)
{
  SetHistStyle(h1,0,0);
  SetHistStyle(h2,3,3);

  TH1D* hr = (TH1D*) h1->Clone(Form("%s_ratio",h1->GetName()));
  hr->Divide(h2);
  TCanvas* c1 = makeHistRatioCanvas(hr,h1,h2,rymin,rymax,islog);
  ...
  SetLegendStyle(leg);
  if(legheader != NULL) leg->SetHeader(Form("%s",legheader.c_str()));
  leg->AddEntry(h1,Form("%s",legname1.c_str()),"pe");
  leg->AddEntry(h2,Form("%s",legname2.c_str()),"pe");
  leg->Draw("same");
  pad1->Update();
  c1->Update();
  c1->SaveAs(Form("%s_wratio.pdf",h1->GetName()));
}
