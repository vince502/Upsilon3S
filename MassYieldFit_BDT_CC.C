...
void MassYieldFit_BDT_CC(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "3p5", const string Trig = "", int cBinLow =0, int cBinHigh = 180, double cutQVP = 0.01, double cutBDTlow=-1, double cutBDThigh = 1. ,Double_t params[/*sigma1S_1, alpha, n , frac, k1, k2, k3, k4*/]= {},Double_t paramslow[/*sigma1S_1, alpha, n , frac, k1, k2, k3, k4*/]= {},Double_t paramshigh[/*sigma1S_1, alpha, n , frac, k1, k2, k3, k4*/]= {}){
  Double_t etaMax= 2.4;
  Double_t etaMin = -2.4;
  ...
  TFile* fout;
  fout = new TFile(Form("/home/vince402/Upsilon3S/Yield/Yield_%s_FF_CC_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.root",ts.c_str(), (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(),(int) swflag, "BDT",1,"cut",cutBDTlow, cutBDThigh ,"vp",cutQVP), "RECREATE");
...
//  RooGenericPdf* bkgErf = new RooGenericPdf("bkgErf", "Error background", "TMath::Exp(-@0/@1)*(TMath::Erf((@0-@2)/(TMath::Sqrt(2)*@3))+1)*0.5", RooArgList(*(works1->var("mass")), Erfp0, Erfmean, Erfsigma));
  ...
  RooChebychev *bkg = new RooChebychev("CCBkg", "Background", *(works1->var("mass")), RooArgSet(ch4_k1, ch4_k2, ch4_k3, ch4_k4));
  ...
  log << "BDT with Chebychev4  NLL : " << NLL; 
  log.close();
  ...
  c1->SaveAs(Form("%s/MassDistribution_CC_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.pdf",massDIR.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDTlow, cutBDThigh,"vp",cutQVP));
  ...
  c1_tmp->SetLogy();
  c1_tmp->Modified();
  c1_tmp->SaveAs(Form("%s/MassDistribution_CC_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.pdf",massDIRl.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDTlow, cutBDThigh,"vp",cutQVP));
