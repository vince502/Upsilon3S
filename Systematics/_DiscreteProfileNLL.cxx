...
pj _DiscreteProfileNLL(ana_bins x = ana_bm_comb["3c"][0]){
...
//		m.minos();
		...
		RooPlot* theframe1 = thevar->frame(200);

		thevar->setConstant();
		for( auto nS : ROOT::TSeqI(0, 1200,1200) ){
			thevar->setVal(nS);
			m.minimize("Minuit", "minimize");
			cv->AddPoint(nS, thenll->getVal());
		}
...
//		RooCurve* retCurve = (RooCurve*) (theframe1->getCurve())->Clone();
		RooCurve* retCurve = cv;
		theframe1->Clear();
		return std::make_pair(npar_offset(type), retCurve);
	};
