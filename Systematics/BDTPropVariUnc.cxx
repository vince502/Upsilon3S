...
pp getBDTPropVariUnc_2item(ana_bins x){
	...
	bp_sys1 = new binplotter(type_sys1, ts,  2.4, pl, ph, cl, ch, 0, bdtud().second, 1, bpl, bph, train_state, state, false, false);
	bp_sys2 = new binplotter(type_sys2, ts,  2.4, pl, ph, cl, ch, 0, bdtud().first, 1, bpl, bph, train_state, state, false, false);
...
double getBDTPropVariUnc(ana_bins x){
	pp res = getBDTPropVariUnc_2item(x);
	return max(fabs(res.first.first), fabs(res.second.first));
};


void BDTPropVariUnc(){
	sys_wr_helper("BDT_Prop_unc.root", getBDTPropVariUnc);
	sys_wr_helper_2item("BDT_Prop_unc_2item.root", getBDTPropVariUnc_2item);
}
