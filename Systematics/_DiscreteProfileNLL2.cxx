...
pj _DiscreteProfileNLL2(ana_bins x = ana_bm_comb["3c"][0]){
	...
	std::string type_sys4 = Form("CB2G:%s:%s",	bkgtype_sys1.c_str(),  fittype.c_str());
	...
	auto name_f_sys4 = GetFit(__FITRESLATEST, false, type_sys4, ts, x.train_state, x.state, x.pl, x.ph, x.cl, x.ch, bl, 1., x.bpl, x.bph, 0.0, "");
	...
	auto f_sys4 = TFile::Open(name_f_sys4.c_str() );

	auto f_write = new TFile(Form("result_discreteProfile/%ld/FullProfile_NLL_%dS_pt%d-%d_cBin%d-%d_%ld%s.root",_TS, x.state, x.pl, x.ph, x.cl, x.ch, _TS, labDR.c_str()), "recreate" );
	...
	auto get_profile = [&] (TFile* input, string pdf, string type){
		...
		if( fittype.find("GC") !=std::string::npos ){
			thefit =(RooFitResult*) input->Get("fitresult_model_gc_reducedDS");
		}
		else{
			thefit =(RooFitResult*) input->Get("fitresult_model_gc_reducedDS");
		}
		RooArgList ll = thefit->floatParsFinal();
		int npar = ll.size();
...
//        fitcmd->Add(&opt13);
		...
		if( fittype.find("GC") !=std::string::npos ){
			...
			opt2 = RooFit::ExternalConstraints(*thearg);
        	fitcmd->Add(&opt2);
		}
		RooAbsReal* thenll =  thepdf->createNLL( *((RooDataSet*) thews->data("reducedDS") ), *fitcmd); 
		
		RooMinimizer m(*thenll);
		m.minimize("Minuit","minimize");
//		m.hesse();
		Double_t minNll = thenll->getVal();
//		m.minos();
		RooRealVar* thevar = (RooRealVar*) thews->var(Form("nSig%dS", x.state));
		thevar->setRange(0, thevar->getVal() + thevar->getError() * 2);
		plt_range = {0, thevar->getVal() + thevar->getError() * 2};
		...
		thepll->plotOn(theframe1,  RooFit::Precision(1) );
		...
		thenll->Clear();
		thepll->Clear();
		RooCurve* retCurve = (RooCurve*) (theframe1->getCurve())->Clone();
		theframe1->Clear();
		return std::make_pair(minNll + 2*npar, retCurve);
	};
	...
	auto Ceval1 =[&](double* x, double* p){
		return min(
			min ( 
				min (2* theres1->Eval(x[0]) + 2 *res1.first , 2* theres2->Eval(x[0]) + 2* res2.first ),
				2* theres3->Eval(x[0]) + 2* res3.first),
			2* theres4->Eval(x[0])+ 2* res4.first );
	};
	...
	for( double pt = plt_range.first; pt < plt_range.second; pt +=( plt_range.second - plt_range.first) /500){
		...
		g_nom->AddPoint(pt, 2* theres1->Eval(pt) + 2*res1.first );
		g_sys1->AddPoint(pt, 2* theres2->Eval(pt)+ 2*res2.first  );
		g_sys2->AddPoint(pt, 2* theres3->Eval(pt)+ 2*res3.first  );
		g_sys3->AddPoint(pt, 2* theres4->Eval(pt)+ 2*res4.first  );
	}
	the_envelop->Write();
