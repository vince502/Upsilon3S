...
#define _TS_LLRINTERNAL 20000000000
...
void LLR_report_static_bl()
{
	...
	auto run_report = [&] (int state) {
		...
		double bl = 0.2620;
		reg = LLR_bkgPDF(findtype(ab,1), findtype(ab,2), ts, ab, bl);
		...
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,1), findtype(ab,3), ts, ab, bl);
		...
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,1), findtype(ab,4), ts, ab, bl);
		...
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,1), findtype(ab,5), ts, ab, bl);
		...
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,1), findtype(ab,6), ts, ab, bl);
		...
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,2), findtype(ab,3), ts, ab, bl);
		pvalue_t[2][3] = reg.first;
		report << Form("p(H0: N=2), 3 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,2), findtype(ab,4), ts, ab, bl);
		pvalue_t[2][4] = reg.first;
		report << Form("p(H0: N=2), 4 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,2), findtype(ab,5), ts, ab, bl);
		pvalue_t[2][5] = reg.first;
		report << Form("p(H0: N=2), 5 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,2), findtype(ab,6), ts, ab, bl);
		pvalue_t[2][6] = reg.first;
		report << Form("p(H0: N=2), 6 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,3), findtype(ab,4), ts, ab, bl);
		pvalue_t[3][4] = reg.first;
		report << Form("p(H0: N=3), 4 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,3), findtype(ab,5), ts, ab, bl);
		pvalue_t[3][5] = reg.first;
		report << Form("p(H0: N=3), 5 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,3), findtype(ab,6), ts, ab, bl);
		pvalue_t[3][6] = reg.first;
		report << Form("p(H0: N=3), 6 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,4), findtype(ab,5), ts, ab, bl);
		pvalue_t[4][5] = reg.first;
		report << Form("p(H0: N=4), 5 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,4), findtype(ab,6), ts, ab, bl);
		pvalue_t[4][6] = reg.first;
		report << Form("p(H0: N=4), 6 : %2.2f %% \n", reg.first * 100); 
		std::cout << "\n P-Value(%) : " << reg.first << "\n\n";
		reg = LLR_bkgPDF(findtype(ab,5), findtype(ab,6), ts, ab, bl);
