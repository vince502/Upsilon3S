...
double getACCVariUnc(int pl, int ph, int cl, int ch, int state){
	...
	hacc_sys = (TH1D*) f_acc_sys->Get("hGenRatio");
	double acc_unc = ( hacc_sys->GetBinContent(1) - hacc_nom->GetBinContent(1) )/ (hacc_nom->GetBinContent(1));
	f_acc_nom->Close();
	f_acc_sys->Close();
	return acc_unc;
};
double getACCVariUnc(ana_bins k){
	double unc_sys = getACCVariUnc(k.pl, k.ph, k.cl, k.ch, k.state);
	return unc_sys;
};

void ACCVariUnc(){
	TFile* output = new TFile("ACC_unc.root","recreate");
	...
	int counter =1;
	for( auto item : vc2){
		rc2s->SetBinContent(counter, getACCVariUnc(item));
		counter++;
	}
	counter =1;
	for( auto item : vc3){
		rc3s->SetBinContent(counter, getACCVariUnc(item));
		counter++;
	}
	counter =1;

	for( auto item : vp2){
		rp2s->SetBinContent(counter, getACCVariUnc(item));
		counter++;
	}
	counter =1;
	
	for( auto item : vp3){
		rp3s->SetBinContent(counter, getACCVariUnc(item));
		counter++;
	}
	output->cd();
	rc2s->Write();
