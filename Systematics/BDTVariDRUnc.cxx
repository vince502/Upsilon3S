...
double getBDTVariUnc(int pl, int ph, int cl, int ch, int state){
	...
	std::string type_sys  = Form("CB3:CC%d:DR%sbdtup",getNomBkgO(state, pl, ph, cl, ch), findtype(pl, ph, cl, ch).c_str());
	std::string type_sys2 = Form("CB3:CC%d:DR%sbdtdown",getNomBkgO(state, pl, ph, cl, ch), findtype(pl, ph, cl, ch).c_str());
	RooRealVar raa_nom, raa_sys, raa_sys2;
	raa_nom = getDoubleRatioValue({cl, ch}, {(double) pl, (double) ph},type_nom, {-2,1}, state, 1, ts, true);
	std::cout << "CHECK" <<  prep_bdtval(1) << std::endl;
	raa_sys = getDoubleRatioValue({cl, ch}, {(double) pl, (double) ph},type_sys, {prep_bdtval(1),1}, state, 1, ts, true);
	raa_sys2 = getDoubleRatioValue({cl, ch}, {(double) pl, (double) ph},type_sys2, {prep_bdtval(-1),1}, state, 1, ts, true);
...
void BDTVariDRUnc(){
	TFile* output = new TFile("BDT_DR_unc.root","recreate");
