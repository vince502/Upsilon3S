...
double getSysPdfVariUnc(int pl, int ph, int cl, int ch, int state){
	...
	std::string type_sys = Form("CB2G:CC%d:DR%s",getNomBkgO(state, pl, ph, cl, ch), findtype(pl, ph, cl, ch).c_str());
...
void SigPdfVariDRUnc(){
	TFile* output = new TFile("sigPDF_DR_unc.root","recreate");
