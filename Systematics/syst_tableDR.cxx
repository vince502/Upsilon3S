...
void syst_tableDR(){
	TFile* f_syst =  TFile::Open(Form("./data/total_systematics_DR_%ld.root", _TS) );
	...
	auto getSyst = [&](string crit, int idx){
		...
		for( int i =0 ; i<6; i++){
			syst_val.push_back(100.*((TH1D*) f_hist->Get(Form("h_r%ss_b%d",crit.c_str(),i )))->GetBinContent((idx-1)*6 +i+1));
	...
	ofstream texout;
	texout.open(Form("syst_tableDR_%ld.tex", _TS) );
	...
	for( auto x : ana_bm_comb ){
		...
		if(abx.state == 2) continue;
			texout << getRow(abx);	
		}
	}
	texout << Form("	\\end{tabular}\n\\end{table}") ;
	texout.close();
	std::cout << Form("BDT & Correction & BKG & HF CENT & SIG PAR & SIG PDF") << std::endl;
