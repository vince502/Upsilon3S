...
double getBkgVariUnc(int pl, int ph, int cl, int ch, int state){
	...
	std::string type_sys = Form("CB3:CC%d:DR%s",getNomBkgO(state, pl, ph, cl, ch)+1, findtype(pl, ph, cl, ch).c_str());
	std::string type_sys2 = Form("CB3:ECC%d:DR%s",getNomBkgO(state, pl, ph, cl, ch), findtype(pl, ph, cl, ch).c_str());
	...
	raa_sys2 = getDoubleRatioValue({cl, ch}, {(double) pl, (double) ph},type_sys2, {-2,1}, state, 1, ts);
...
void BkgVariDRUnc(){
	TFile* output = new TFile("bkgPDF_DR_unc.root","recreate");
