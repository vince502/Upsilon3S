...
void MassYieldFit_BDT_fix(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "4", const string Trig = "", int cBinLow =0, int cBinHigh = 180, double cutQVP = 0.01, double cutBDT=-1 ,Double_t params[/*sigma1S_1, alpha, n , frac, Erfmean, Erfsigma, Erfp0*/]= {}){
  Double_t etaMax= 2.4;
  Double_t etaMin = -2.4;
  ...
  TString massDIR = mainDIR +  Form("/MassDist/BDT/%s/DatafixtoMC/%s/%s/FitResult",ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIRp = mainDIR +  Form("/MassDist/BDT/%s/DatafixtoMC/%s/%s/FitResult/png", ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIRl = mainDIR + Form("/MassDist/BDT/%s/DatafixtoMC/%s/%s/FitResult/logY", ts.c_str(), Trig.c_str(), MupT.Data());
  TString massDIR2 = mainDIR + Form("/MassDist/BDT/%s/DatafixtoMC/%s/%s/WithoutFit", ts.c_str(), Trig.c_str(), MupT.Data());
  ...
  TFile* fout;
  fout = new TFile(Form("Yield/Yield_%s_DFM_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f_%s%.4f.root",ts.c_str(), (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(),(int) swflag, "BDT",1,"cut",cutBDT,"vp",cutQVP), "RECREATE");
