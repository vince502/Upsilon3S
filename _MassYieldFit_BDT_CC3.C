...
void MassYieldFit_BDT_CC3(const string fname = "", const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const TString MupT = "3p5", const string Trig = "", int cBinLow =0, int cBinHigh = 180, double cutQVP = 0.01, double cutBDTlow=-1, double cutBDThigh = 1. ,Double_t params[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {},Double_t paramslow[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {},Double_t paramshigh[/*sigma1S_1, alpha, n , frac, k1, k2, k3*/]= {}){



  massfitter mf = massfiter();
...
//  TFile* fout;
//  fout = new TFile(Form("/home/vince402/Upsilon3S/Yield/Yield_%s_FF_CC3_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.root",ts.c_str(), (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(),(int) swflag, "BDT",1,"cut",cutBDTlow, cutBDThigh ,"vp",cutQVP), "RECREATE");
//  TFile* fin;
//  fin = new TFile(Form("%s",fname.c_str()),"READ");
  mf.massfitter();
  Double_t MupTCut = LepPtCut(MupT);
...
//on RooGaussian* G3S = new RooGaussian("G3S", "3S Gaussian", *(works1->var("mass")), mean3S, sigma3S_2);
...
//  RooChebychev *bkg = new RooChebychev("CCBkg", "Background", *(works1->var("mass")), RooArgSet(ch4_k1, ch4_k2, ch4_k3));
  ...
  fit_model_ups::ChebyChev* cc3 = new fit_model_ups::ChevyChev(*(mf.works->var("mass")), &ch4_k1, &ch4_k2, &ch4_k3));
...
//  ch4_k4.setVal(params[7]);
  ...
  c1->SaveAs(Form("%s/MassDistribution_CC3_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.pdf",massDIR.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDTlow, cutBDThigh,"vp",cutQVP));
  c1->SaveAs(Form("%s/png/MassDistribution_CC3_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.png",massDIR.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDTlow, cutBDThigh,"vp",cutQVP));
  TCanvas* c1_tmp = (TCanvas*) c1->Clone("ylogc1");
  c1_tmp->SetLogy();
  c1_tmp->Modified();
  c1_tmp->SaveAs(Form("%s/MassDistribution_CC3_pt_%d-%d_rap_%d-%d_%dbin_cbin_%d-%d_MupT%s_Trig_%s_SW%d_%s%d_%s%.4f-%.4f_%s%.4f.pdf",massDIRl.Data(), (int)(ptMin*10), (int)(ptMax*10), (int)(rapMin*10), (int)(rapMax*10), Nmassbins, cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), (int) swflag,"BDT",1,"cut",cutBDTlow, cutBDThigh,"vp",cutQVP));
