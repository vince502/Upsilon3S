...
void getAcceptance_V2(double ptLow = 0, double ptHigh=50, double yLow=0, double yHigh=2.4, double SiMuPtCut=3.5, bool isPtWeight= true,  int state =3, int IsSys = 0)
{
  gStyle->SetOptFit(kTRUE);

  TFile* f, *fPtW;
    if(state ==1){	f =new TFile("/home/CMS/DataFiles/5TeVUpsilonGENONLY/OniaTree_Ups1SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root","read");
	fPtW = new TFile(Form("%s/WeightedFunction/Func_dNdpT_1S.root",store.Data()),"read");
    }
    if(state ==2){	f =new TFile("/home/CMS/DataFiles/5TeVUpsilonGENONLY/OniaTree_Ups2SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root","read");
	fPtW = new TFile(Form("%s/WeightedFunction/Func_dNdpT_2S.root",store.Data()),"read");
    }
    if(state ==3){
    	f =new TFile("/home/CMS/DataFiles/5TeVUpsilonGENONLY/OniaTree_Ups3SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root","read");
  ...
  TString histName = Form("y%.1f_%.1f_SimuPt%.1f_ptW%d",yLow,yHigh,SiMuPtCut, (int) isPtWeight);
  if( IsSys != 0) histName = histName +"_SYSPT"; 

  TFile *wf = new TFile(Form("AccRes_%dS_%s.root",state,histName.Data()),"recreate");

  TH1D* hGen = new TH1D("hGen",";pT;",50,0,50);
  TH1D* hGen_NU = new TH1D("hGen_NU",";pT;",50,0,50);
  TH1D* hGen_ND = new TH1D("hGen_ND",";pT;",50,0,50);
  TH1D* hGen_UN = new TH1D("hGen_UN",";pT;",50,0,50);
  TH1D* hGen_DN = new TH1D("hGen_DN",";pT;",50,0,50);
  TH1D* hGenAcc = new TH1D("hGenAcc",";pT;",50,0,50);
  TH1D* hGenAcc_NU = new TH1D("hGenAcc_NU",";pT;",50,0,50);
  TH1D* hGenAcc_ND = new TH1D("hGenAcc_ND",";pT;",50,0,50);
  TH1D* hGenAcc_UN = new TH1D("hGenAcc_UN",";pT;",50,0,50);
  TH1D* hGenAcc_DN = new TH1D("hGenAcc_DN",";pT;",50,0,50);
