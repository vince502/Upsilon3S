...
auto prep_bdtval = [&] (double blow_ref = -0.3, int _step =0) mutable {
  ...
  RooRealVar sb_ratio;
    if( _step > -10 ){
    sb_ratio = get_eff_acc(type_r, typenobdt, ts, rapMax, ptMin, ptMax, cBinLow, cBinHigh, blow_ref, 1.0, train_state, 1, state);
 ...
 for( auto rp : rapPair ) {
   for( auto pp : ptPair ) {
     for( auto cp : cBinPair ){
      ...
      long xts = ts;
    std::pair<double, RooRealVar> res = prep_bdtval(-0.1, -1);
//    std::pair<double, RooRealVar> res0 = prep_bdtval(-0.1, -102);
//    std::pair<double, RooRealVar> res1 = prep_bdtval(-0.1, -101);
//    std::pair<double, RooRealVar> res2 = prep_bdtval(-0.1, -100);
    cutBDTlow = res.first;
...
if(step ==2 || step ==99|| step ==21){
  ...
  for( auto ptpair : (std::vector<std::pair<double, double> >) {{0, 30},/* {0, 4}, {4, 9},{9, 30}*/ }){ 
    ...
    ptMax = ptpair.second;
   std::pair<double, RooRealVar> res = prep_bdtval(-0.0, -1);
//    std::pair<double, RooRealVar> res0 = prep_bdtval(-0.0, -102);
//    std::pair<double, RooRealVar> res1 = prep_bdtval(-0.0, -101);
//    std::pair<double, RooRealVar> res2 = prep_bdtval(-0.0, -100);
    cutBDTlow = res.first;
    sb_ratio = res.second;
    if(ptpair.first == 0 && ptpair.second == 30){ INTBIN_BDTLOW = res.first; }
    std::cout << "cutBDTlow, sb_ratio: " << cutBDTlow << ", "<< sb_ratio.getVal() << std::endl;
