...
#include "MassYieldFit_BDT_MC_CB1.C"
#include "MassYieldFit_BDT_MC_CB2.C"
...
void doConstraintFit_NOM_combv2_forCB12(int step = 0){
...
//////////////////////////////////////////STEP : GAUS CONSTRAIN FITTING/////////////////////////////////////////
  ...
  std::cout << "cutBDTlow, sb_ratio in functoin : " << cutBDTlow << ", " << sb_ratio.getVal() << std::endl;
    ...
    if(mode ==1 || mode ==0){
       MassYieldFit_BDT_MC_CB1(ts, "", fname, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, train_state, state, fixvar, swflag, cutQVP, cutBDTlow, cutBDThigh, bdtptMin, bdtptMax,plot_dir_opt);
    }
  ...
  auto METHOD_MCGCDATA1 = [&](int mode = 1) mutable {
  std::cout << "cutBDTlow, sb_ratio in functoin : " << cutBDTlow << ", " << sb_ratio.getVal() << std::endl;
    std::string fname;
    if(state ==1) fname = fname1S;
    if(state ==2) fname = fname2S;
  ...
  auto METHOD_MCGCDATA2 = [&](int mode = 1) mutable {
  std::cout << "cutBDTlow, sb_ratio in functoin : " << cutBDTlow << ", " << sb_ratio.getVal() << std::endl;
    std::string fname;
    if(state ==1) fname = fname1S;
    if(state ==2) fname = fname2S;
	...
	std::cout << Form("%ld,%.4f, %.4f, %d, %d, %d, %d, %.4f, %.4f, %d, %d", ts, ptMin, ptMax, cBinLow, cBinHigh, train_state, state, cutBDTlow, cutBDThigh, bdtptMin, bdtptMax) << std::endl;
    if(mode ==1 || mode ==0){
       MassYieldFit_BDT_MC_CB2(ts, "", fname, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, train_state, state, fixvar, swflag, cutQVP, cutBDTlow, cutBDThigh, bdtptMin, bdtptMax,plot_dir_opt);
    }
...
//	if(!(cBinLow == 140 && cBinHigh == 181 ) ) continue;
	...
	if(fitdata){
	    ...
	    bkg_low  = {6.5, 1.1, 0.01,  -0.5, -0.5, -0.5, -0.5, -0.5, -0.5};
	    bkg_high = {8.6, 15.0, 15.0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5};
		fixfit(CBINBDTLOW);
		if(fit_aux){
