...
// std::string constraints		="alpha:n:frac:x3S:sigma3S_1"	;
// std::string fixvars			="alpha:n:frac:x1S:sigma1S_1"	;
  long ts				= 1625558168; // 1625139244; // Past nominal 1623391157; //71622626134;//	1621574976;//	//	1622517421;
...
/*
*/
auto prep_bdtval = [&] (double blow_ref = -0.3, int step =0) mutable {
  if(step >=3 || step == -103 || step == -51){
       MassYieldFit_BDT_MC_CB3(ts, "", fname3S, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, state, fixvar, swflag, cutQVP, -1, cutBDThigh);
//       MassYieldFit_BDT_MC(ts, "", fname3S, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, state, fixvar, swflag, cutQVP, -1, cutBDThigh);
  }
  if(step >=2|| step == -102 || step == -52){
       MassYieldFit_BDT_MC_CB3(ts, "", fname3S, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, state, fixvar, swflag, cutQVP, blow_ref, cutBDThigh);
//       MassYieldFit_BDT_MC(ts, "", fname3S, ptMin, ptMax, rapMin, rapMax, MupT, Trig, cBinLow, cBinHigh, state, fixvar, swflag, cutQVP, blow_ref, cutBDThigh);
}
...
//      TFile* file_MCres_input = new TFile(Form("Yield/Yield_%ld_CB3_%dS_pt_%d-%d_rap_%d-%d_cBin_%d-%d_MupT%s_%s_BDT_%.4f-%.4f_vp_%.4f_MC_%d.root", ts, (int) state ,(int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), bl, cutBDThigh, cutQVP,(int) fixvar), "OPEN");
  ...
  if(step >=1|| step == -101 || step == -51){
    fit_mc_dat(typenobdt, -1);
  }
  if(step >=0|| step == -100 || step == -52){
   ...
   double res_blow = Get_Optimal_BDT(ts,ptMin, ptMax, rapMin, rapMax, cBinLow, cBinHigh, cutQVP , sb_ratio.getVal(), "", "S2").first;
   if(step <=-999){
   auto h1 =func_hist_optimal_BDT();
   TCanvas* c1 =new TCanvas();
   c1->cd();
   h1->Draw();
   }
   std::cout << res_blow << " is the determined BDT \n\n\n\n\n\n\n\n\n" << std::endl;
...
//      TFile* file_MCres_input = new TFile(Form("Yield/Yield_%ld_CB2_%dS_pt_%d-%d_rap_%d-%d_cBin_%d-%d_MupT%s_%s_BDT_%.4f-%.4f_vp_%.4f_MC_%d.root", ts, (int) state ,(int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), cBinLow, cBinHigh, MupT.Data(), Trig.c_str(), cutBDTlow, cutBDThigh, cutQVP,(int) fixvar), "OPEN");
      ...
      std::cout << type << std::endl;
       ...
       double mean_sigma1S1 =  map_keyval["sigma3S_1"].first*(U1S_mass/U3S_mass);
      MassYieldFit_data(type, ptMin, ptMax, rapMin, rapMax, MupT, Trig, swflag, cBinLow, cBinHigh, cutQVP, isBDT, ts, cutBDTlow, cutBDThigh, (Double_t[]) {mean_sigma1S1, map_keyval["alpha"].first, map_keyval["n"].first, map_keyval["frac"].first, -0.15,-0.05,0.03,0.03, -0.03}, (Double_t[]) {0.11, 0.5, 0.4, 0.05, -0.1, -0.2, -0.1,-0.1, -0.1}, (Double_t[]) {0.41, (map_keyval["alpha"].first+map_keyval["alpha"].second)*2, (map_keyval["n"].first+map_keyval["n"].second)*4, 0.95, 0.0, 0.0, 0.2,0.1, 0.1}, 
...
//std::pair<double, RooRealVar> res = prep_bdtval(-0., -1);
//cutBDTlow = res.first;
////cutBDTlow = -0.1;
//sb_ratio = res.second;
...
auto fixfit = [&]() mutable {
 ...
 std::vector<std::pair<int, int> > cBinPair =	{ {0,40}, {40, 100}, /* {100,181} */ }; // {0,10}, {10,20}, {20,40}, {40,60}, {60,80}, {80,100}, {100,120}, {120, 140}, {140, 180} };	//{ {0,40}, {40, 100}, {100,180}, {0, 180} };
...
//    cutBDTlow = idx;
    ...
    sb_ratio = res.second; //RooRealVar("x","",-1); //     std::cout << "cutBDTlow, sb_ratio: " << cutBDTlow << ", "<< sb_ratio.getVal() << std::endl;
