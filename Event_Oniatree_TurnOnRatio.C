...
void Event_Oniatree_TurnOnRatio(int nevt=-1, bool isMC = false, int kTrigSel = 0, int hiHFBinEdge = 0, int PDtype = 1) 
{
...
//  mytree->SetBranchAddress("Reco_mu_ptErr_global", Reco_mu_ptErr_global, &b_Reco_mu_ptErr_global);
  ...
  TFile* newfile;
  newfile = new TFile("MBTrigCentPlots_2p9m.root","recreate");
  ...
  cBin = getHiBinFromhiHF(SumET_HF);
    if( (HLTriggers&((ULong64_t)pow(2, kTrigSel))) == ((ULong64_t)pow(2, kTrigSel)) ){
       h1->Fill(cBin);
       if( (HLTriggers&((ULong64_t)pow(2,1))) == ((ULong64_t)pow(2,1)) ) h2->Fill(cBin);
       if( (HLTriggers&((ULong64_t)pow(2,2))) == ((ULong64_t)pow(2,2)) ) h3->Fill(cBin);
    }
